...
$L['trade_message_c5'] = '<a href="{V0}" class="t">{V1}</a> 于 <span class="f_gray">{V2}</span> 向您支付了 <span class="f_blue">{V3}'.$AJ['money_unit'].'</span> 的站内付款<br/>备注：<span class="f_gray">{V4}</span>';
$L['trade_message_t6'] = '站内交易提醒，您有一笔交易需要确认(T{V0})';
$L['trade_message_c6'] = '<a href="{V0}" class="t">{V1}</a> 于 <span class="f_gray">{V2}</span> 向您订购了：<br/>{V3}<br/>订单编号：<span class="f_red">T{V4}</span> &nbsp;订单金额为：<span class="f_blue f_b">{V5}'.$AJ['money_unit'].'</span><br/><a href="{V6}" class="t" target="_blank">&raquo; 请点这里立即处理或查看详情</a>';
