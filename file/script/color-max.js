...
document.write('.color_div_o {width:16px;height:16px;padding:6px;background:#DDDDDD;cursor:crosshair;}');
...
color_htm += '</tr>';
color_htm += '</table>';
color_htm += '<div id="aijiacms_color_show"></div>';
function color_show(id, color, obj) {
	Eh();
	if(Dd('aijiacms_color') == null) {
		var aijiacms_color_div = document.createElement("div");
		aijiacms_color_div.id = 'aijiacms_color';
		document.body.appendChild(aijiacms_color_div);		
		$('#aijiacms_color').css({'zIndex':'9999','position':'absolute','display':'none','width':'226px','background':'#FFFFFF'});
	}
	$('#aijiacms_color').css({'left':$('#color_img_'+color_id).offset().left+'px','top':($('#color_img_'+color_id).offset().top+$('#color_img_'+color_id).height())+'px'});
	$('#aijiacms_color').html(color_htm);
	color_id = id;
	color_bk = color;
	$('#aijiacms_color').fadeIn(300);
	if(color) color_view(color);
	color_setup(color);
}
function color_hide() {$('#aijiacms_color').fadeOut(300);Es();}
function color_close() {color_hide();Dd('color_img_'+color_id).style.backgroundColor = color_bk;}
function color_select(color) {color=color.toUpperCase();if(!color.match(/^#[A-F0-9]{6}$/)){return;}Dd('color_input_'+color_id).value = color; Dd('color_img_'+color_id).style.backgroundColor = color; color_hide();}
function color_setup(color) {
	...
	for(i = 0; i < colors.length; i++) {
		...
		colors_select += '<div class="color_div" style="background:'+(colors[i] ? colors[i] : '#FFFFFF;border:#DDDDDD 1px dotted;')+'">&nbsp;</div></div></td>';
		if(i%8 == 7) colors_select += '</tr>';
	}
	colors_select += '</table>';
	$('#aijiacms_color_show').html(colors_select);
}
function color_view(color){color=color.toUpperCase();if(!color.match(/^#[A-F0-9]{6}$/)){return;}try {Dd('color_viewview').value = color; Dd('color_viewview').style.color = color; Dd('color_img_'+color_id).style.backgroundColor = color;} catch(e) {}}