...
function ca_setup(year,month){
	...
	for(i=0;i<7;i++){calendar += '<td style="width:32px;height:32px;background:#F1F1F1;font-weight:bold;font-size:13px;">'+(weeks[i])+'</td>';}
	calendar += '</tr>';
	l=cells.length
	for(i=0;i<l;i++){
		if(i%7==0){calendar += '<tr>';}
		if(cells[i]){
			calendar += '<td style="cursor:pointer;height:32px;font-size:13px;border-top:#E0E0E0 1px solid;'+(i%7==6?'':'border-right:#E0E0E0 1px solid;')+'';
