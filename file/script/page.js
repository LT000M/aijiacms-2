...
function show_answer(u, i) {document.write('<iframe src="'+u+'answer.php?itemid='+i+'" name="aijiacms_answer" id="des'+'toon_answer" style="width:100%;height:0px;" scrolling="no" frameborder="0"></iframe>');}
//function show_task(s) {document.write('<script type="text/javascript" src="'+DTPath+'api/task.js.php?'+s+'&refresh='+Math.random()+'.js"></sc'+'ript>');}
function show_task(p, s) {$.getScript(DTPath+'api/task.js.php?'+p+(s ? '&screenw='+window.screen.width+'&screenh='+window.screen.height+'&refer='+encodeURIComponent(document.referrer) : '')+'&refresh='+Math.random()+'.js');}
var sell_n = 0;
...
function Dqrcode() {
	...
	if($('#aijiacms_qrcode').length > 0) {
		if($('#aijiacms_qrcode').html().length < 10) {
			$('#aijiacms_qrcode').css({'position':'fixed','z-index':'99999','left':'50%','top':'0','margin-left':'-130px','width':'260px','background':'#FFFFFF','text-align':'center'});
			$('#aijiacms_qrcode').html('<div style="text-align:right;color:#555555;font-size:16px;font-family:Verdana;font-weight:100;padding-right:6px;cursor:pointer;">x</div><img src="'+DTPath+'api/qrcode.png.php?auth='+encodeURIComponent(url)+'" width="140" height="140"/><div style="padding:10px 0;font-size:14px;font-weight:bold;color:#555555;">扫一扫，直接在手机上打开</div><div style="padding-bottom:20px;color:#999999;">推荐微信、QQ扫一扫等扫码工具</div>');
			$('#aijiacms_qrcode').click(function(){$('#aijiacms_qrcode').fadeOut('fast');});
		}
		$('#aijiacms_qrcode').fadeIn('fast');
	}
}
function Dmobile() {
	var url = $('meta[http-equiv=mobile-agent]').attr('content');
