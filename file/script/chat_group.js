...
function chat_send(msg){
	if(chat_ban){chat_tip('您已被禁言');return;}
...
function chat_load(d){
	$.post('chat.php', 'action=load&chatlast='+chat_last+'&gid='+chat_gid+'&first='+(d ? 1 : 0), function(data) {
		if(data) {
			...
			if(msglen && d) {$('#chat').append('<div class="chat-more"><a href="'+MEPath+'im.php?action=view&mid='+chat_mid+'&gid='+chat_gid+'" target="_blank"><span>更多记录</span></a></div>');}
