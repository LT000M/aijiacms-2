...
if($AJ_MOB['os'] == 'ios') {
	if(preg_match("/^([0-9]{1,})@([a-z0-9]{16,})$/i", $EXT['mobile_ios'])) {
		...
		dheader('https://app.aijiacms.com/get.php?o=ios&u='.$t[0].'&k='.encrypt($url, $t[1]));
	} else if(strpos($EXT['mobile_ios'], 'itunes.apple.com') !== false) {
		if($AJ_MOB['browser'] == 'weixin') {			
			$html = '';
			...
			$html .= '<img src="'.AJ_PATH.'file/image/weixin-ios.png" style="width:300px;"/>';
			$html .= '</body></html>';
			exit($html);
		}
	...
	if(preg_match("/^([0-9]{1,})@([a-z0-9]{16,})$/i", $EXT['mobile_adr'])) {
		...
		dheader('https://app.aijiacms.com/get.php?o=adr&u='.$t[0].'&k='.encrypt($url, $t[1]));
		...
		if($AJ_MOB['browser'] == 'weixin') {			
			...
			$html .= '<title>Tips</title></head>';
			$html .= '<body style="background:#FFFFFF;text-align:center;">';
			$html .= '<img src="'.AJ_PATH.'file/image/weixin-adr.png" style="width:300px;"/>';
			$html .= '</body></html>';
			exit($html);
		}
