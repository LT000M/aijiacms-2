...
switch($action) {
	...
	case 'delete':
		...
		if($r) {
			...
			$forward = AJ_PATH.'api/comment.php?mid='.$mid.'&itemid='.$itemid.'&page='.$page.'&proxy='.$proxy.'&rand='.mt_rand(10, 99);
		...
		if($EXT['comment_api']) {
			...
			if($submit) {
				...
				if($status == 3) {
					...
					if($MOD['comment_show']) {
						$forward = AJ_PATH.'api/comment.php?mid='.$mid.'&itemid='.$itemid.'&page='.$page.'&proxy='.$proxy.'&rand='.mt_rand(10, 99).'#last';
