...
define('SDK_SIGN_CERT_PATH', AJ_ROOT.'/api/pay/'.$bank.'/'.($PAY[$bank]['cert'] ? $PAY[$bank]['cert'] : 'zhengshu.pfx'));

// 签名证书密码
define('SDK_SIGN_CERT_PWD', $PAY[$bank]['keycode']);

// 密码加密证书（这条一般用不到的请随便配）
define('SDK_ENCRYPT_CERT_PATH', AJ_ROOT.'/api/pay/'.$bank.'/acp_prod_enc.cer');

// 验签证书路径（请配到文件夹，不要配到具体文件）
define('SDK_VERIFY_CERT_DIR', AJ_ROOT.'/api/pay/'.$bank.'/');
...
define('SDK_BACK_NOTIFY_URL', AJ_PATH.'api/pay/'.$bank.'/'.($PAY[$bank]['notify'] ? $PAY[$bank]['notify'] : 'notify.php'));

//文件下载目录 
define('SDK_FILE_DOWN_PATH', AJ_ROOT.'/api/pay/'.$bank.'/file/');

//日志 目录 
define('SDK_LOG_FILE_PATH', AJ_ROOT.'/api/pay/'.$bank.'/logs/');

//日志级别，关掉的话改PhpLog::OFF
define('SDK_LOG_LEVEL', 'PhpLog::OFF');


/** 以下缴费房源使用，其余房源用不到，无视即可 */
// 前台请求地址
