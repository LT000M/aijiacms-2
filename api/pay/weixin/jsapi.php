...
$charge_title = '';
$auth = isset($auth) ? decrypt($auth, AJ_KEY.'JSPAY') : '';
$auth or dheader($MODULE[2]['mobile'].'charge.php?action=record');
$t = explode('|', $auth);
$itemid = $orderid = intval($t[0]);
$openid = $t[3];
($itemid && $t[2] == $AJ_IP && is_openid($openid)) or dheader($MODULE[2]['mobile'].'charge.php?action=record');
$charge_title = $t[1];
...
<div style="width:100%;text-align:center;">
	...
	<div style="line-height:24px;font-weight:bold;"><span style="font-size:18px;"><?php echo $AJ['money_sign'];?></span><span style="font-size:22px;"><?php echo str_replace('.', '</span><span style="font-size:16px;">.', strpos($charge, '.') === false ? $charge.'.00' : $charge);?></span></div>
