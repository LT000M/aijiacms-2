...
if($wx->signature()) {
	...
	if($HTTP_RAW_POST_DATA) {
		...
		$post['addtime'] = $AJ_TIME;//$CreateTime;
		$post['type'] = $MsgType;
		$post['misc'] = array();
		if($MsgType == 'event') {//事件
			$Event = $x->Event;
			...
			switch($Event) {
				case 'CLICK'://点击菜单拉取消息时的事件推送
					switch($EventKey) {
						case 'V_member'://绑定会员
							...
							$tags['picurl'] = AJ_PATH.'api/weixin/image/top_bind.jpg';
							$tags['url'] = AJ_MOB.'api/weixin.php?action=member&auth='.encrypt("$FromUserName", AJ_KEY.'WXID');
							$misc[] = $tags;						
						...
						default:
							if(substr($EventKey, 0, 5) == 'V_mid') {
								...
								if(in_array($mod, array('article', 'sale', 'rent', 'newhouse', 'group', 'job',  'photo', 'special', 'video'))) {
									$post['content'] = $EventKey;
									$misc = array();
								...
								} else if(in_array($mod, array('newhouse','sale', 'buy', 'info', 'rent'))) {
									$post['content'] = $EventKey;
									$misc = array();
				...
				case 'subscribe'://订阅
					...
					$stime > 0 or $stime = $AJ_TIME;
					$sql = "subscribe=1,addtime=$stime,edittime=$AJ_TIME,visittime=$AJ_TIME";
					foreach(array('nickname', 'sex', 'city', 'province', 'country', 'language', 'headimgurl') as $v) {
						if(isset($info[$v])) $sql .= ",".$v."='".addslashes($info[$v])."'";
					}
					if($user) {
						$db->query("UPDATE {$AJ_PRE}weixin_user SET $sql WHERE openid='$FromUserName'");
					} else {
						$sql .= ",openid='$FromUserName'";
						$db->query("INSERT INTO {$AJ_PRE}weixin_user SET $sql");
					}
					if(strpos($post['content'], 'qrscene_') !== false) {
						$sid = intval(substr($post['content'], 8));
						$B = $db->get_one("SELECT * FROM {$AJ_PRE}weixin_bind WHERE sid='$sid'");
						if($B) {
							if($AJ_TIME - $B['addtime'] < 1800 && check_name($B['username'])) weixin_bind($FromUserName, $B['username']);
							$db->query("DELETE FROM {$AJ_PRE}weixin_bind WHERE sid='$sid'");
						}
					}
					//回复欢迎消息
				...
				case 'unsubscribe'://取消订阅
					$post['content'] = '';
					$db->query("UPDATE {$AJ_PRE}weixin_user SET subscribe=0,username='',edittime=$AJ_TIME WHERE openid='$FromUserName'");
				break;
				case 'SCAN'://扫描二维码[已关注]
					$post['content'] = $EventKey;
					$post['misc']['Ticket'] = "$x->Ticket";
			...
			switch($MsgType) {
				case 'text'://文本消息
					...
					if($Content == '签到') {
						...
						//自动回复
						$t = $db->get_one("SELECT * FROM {$AJ_PRE}weixin_auto WHERE keyword='$Content'");
						if($t) {
		...
		if($credit_add && $WX['credit']) {//签到送积分
			...
			if($credit && $user && $user['username'] && timetodate($AJ_TIME, 3) != timetodate($user['credittime'], 3)) {
				require_once AJ_ROOT.'/include/module.func.php';
				credit_add($user['username'], $credit);
				credit_record($user['username'], $credit, 'system', '微信签到');
				$db->query("UPDATE {$AJ_PRE}weixin_user SET credittime=$AJ_TIME WHERE itemid=$user[itemid]");
				$msg = '签到成功，已赠送您'.$credit.$AJ['credit_name'];
			}
			$wx->response($FromUserName, $ToUserName, 'text', $msg);
		}
		$db->query("UPDATE {$AJ_PRE}weixin_user SET visittime=$AJ_TIME WHERE openid='$FromUserName'");
	} else {
		echo $_GET["echostr"];
	}
} else {
	echo AJ_DEBUG ? 'Working...' : '<meta http-equiv="refresh" content="0;url=../">';
}
?>