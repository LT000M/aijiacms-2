...
include_once AJ_ROOT.'/api/weixin/config.inc.php';
...
class weixin {
	...
	function get_token() {
		...
		if($wt && $wt['token'] && AJ_TIME - $wt['time'] < 7000) return $wt['token'];
		...
		cache_write('weixin-token.php', array('time' => AJ_TIME, 'token' => $access_token));
		return $access_token;
	}
	...
	function response($openid, $from, $type, $content, $misc = array()) {
		...
		$xml .= '<CreateTime>'.AJ_TIME.'</CreateTime>';
...
function weixin_user($openid) {
	return DB::get_one("SELECT * FROM ".AJ_PRE."weixin_user WHERE openid='$openid'");
}

function weixin_bind($openid, $username) {
	if(check_name($username)) {
		DB::query("UPDATE ".AJ_PRE."weixin_user SET username='' WHERE username='$username'");
		DB::query("UPDATE ".AJ_PRE."weixin_user SET username='$username' WHERE openid='$openid'");
	}
}
