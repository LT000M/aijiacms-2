...
switch($action) {
	case 'login':
		...
		$user = $db->get_one("SELECT username,passport,password,groupid,email,passsalt FROM {$AJ_PRE}member WHERE username='$username'");
		...
		if($uid == -1) {/* Ucenter 用户不存在或被删除 */
			...
			if($user) {
				...
				$auth = encrypt($username.'|'.$rt_password.'|'.$rt_email, AJ_KEY.'UC');
				/*uc登录自动激活账号2022-7-25*/
		$post['groupid'] = $post['regid'] = 5;
			$post['username'] = $username;
			$post['passport'] = $username;	
		$do = new member;
		$do->add($post);
				dheader($MOD['linkurl'].$AJ['file_register'].'?action=success&auth='.encrypt('LOGIN|'.$post['username'].'|'.$password.'|'.$AJ_TIME, AJ_KEY.'LOGIN'));
			/*end*/
			}
		}
	break;
