...
if($job == 'item') {
	...
	isset($from) or $from = '';
	if($AJ['uploadlog']) {
		$table = $AJ_PRE.'upload_'.($_userid%10);
		...
		if($itemid) {
			$item = $db->get_one("SELECT username,title FROM {$AJ_PRE}photo_{$mid} WHERE itemid=$itemid");
			if(!$item || ($item['username'] != $_username && $_groupid > 1)) dheader(AJ_PATH);
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}photo_item_{$mid} WHERE item=$itemid");
			$items = $r['num'];
			$pages = pages($items, $page, $pagesize);
			$lists = array();
			$result = $db->query("SELECT * FROM {$AJ_PRE}photo_item_{$mid} WHERE item=$itemid ORDER BY listorder ASC,itemid ASC LIMIT $offset,$pagesize");
			while($r = $db->fetch_array($result)) {
			...
			if($keyword) $condition .= " AND keyword LIKE '%$keyword%'";
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}photo_{$mid} WHERE $condition");
			$items = $r['num'];
			$pages = pages($items, $page, $pagesize);
			$lists = array();
			$result = $db->query("SELECT * FROM {$AJ_PRE}photo_{$mid} WHERE $condition ORDER BY addtime LIMIT $offset,$pagesize");
			while($r = $db->fetch_array($result)) {
				$lists[] = $r;
			}
		}
