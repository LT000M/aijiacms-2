...
switch($job) {
	...
	case 'get_company':
		...
		if($user) {
			echo '<a href="'.$user['linkurl'].'" target="_blank" class="t">'.$user['company'].'</a>'.( $user['vip'] ? ' <img src="'.AJ_SKIN.'image/vip.gif" align="absmiddle"/> <img src="'.AJ_SKIN.'image/vip_'.$user['vip'].'.gif" align="absmiddle"/>' : '');
		} else {
			echo '1';
		}
		exit;
