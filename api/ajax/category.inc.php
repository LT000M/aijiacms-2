...
$category_extend = isset($category_extend) ? decrypt($category_extend, AJ_KEY.'CAT') : '';
...
if($_groupid == 1 && $_admin == 2) {
	...
	if(isset($R[$category_moduleid]['index']['catid'])) {
		...
		if($_catids && is_array($_catids)) {
			$_childs = '';
			$result = $db->query("SELECT arrchildid FROM {$AJ_PRE}category WHERE catid IN (".implode(',', $_catids).")");
