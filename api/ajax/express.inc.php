...
if($AJ['cloud_express'] && AJ_CLOUD_UID && AJ_CLOUD_KEY) {
	$cache = 0;
	$cid = AJ_ROOT.'/file/cloud/express/'.urlencode($_com).'/'.substr($no, 0, 3).'/'.substr($no, -3).'/'.$no.'.php';
	if(is_file($cid)) {
	...
	if($arr['error']) {
		...
		if($arr['data']) {
			...
			if($htm) $htm .= '<div>- 以上数据由'.($arr['url'] ? '<a href="'.AJ_PATH.'api/redirect.php?url='.urlencode($arr['url']).'" class="t" target="_blank">' : '').$arr['name'].($arr['url'] ? '</a>' : '').'提供，如有疑问请到<a href="'.AJ_PATH.'api/express.php?action=home&e='.urlencode($com).'&n='.$no.'" class="t" target="_blank">快递公司官网</a>查询</div>';
		}
	}
}
if($status < $_status) {
	if($from == 'mall') {
		$db->query("UPDATE {$AJ_PRE}order SET send_status=$_status WHERE itemid=$itemid");
	} else if($from == 'group') {
		if($mid && $MODULE[$mid]['module'] == $from) $db->query("UPDATE {$AJ_PRE}{$from}_order_{$mid} SET send_status=$_status WHERE itemid=$itemid");
	}
}
