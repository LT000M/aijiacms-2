...
$_POST['housetype']=$housetype;
$db->query("UPDATE {$AJ_PRE}member SET message=message+1 WHERE username='$touser'");
  $db->query("INSERT INTO {$AJ_PRE}message (title,typeid,touser,fromuser,content,addtime,ip,status,telephone,truename,sex,email,linkurl) VALUES ('$title', '$housetype','$touser','$fromuser','$content','$AJ_TIME','$AJ_IP',3,'$telephone','$truename','$sex','$email','$linkurl')");
...
$db->query("UPDATE {$AJ_PRE}member SET message=message+1 WHERE username='$touser'");
  $db->query("INSERT INTO {$AJ_PRE}message (title,typeid,touser,fromuser,content,addtime,ip,status,telephone,truename,sex,email,linkurl) VALUES ('$title', 1,'$touser','$fromuser','$content','$AJ_TIME','$AJ_IP',3,'$telephone','$truename','$sex','$email','$linkurl')");
       ...
       echo json_encode(array('state'=>'succ','alert'=>'发送成功！'));
}
	   else
...
if($action == 'wapgroup'){
$_GET['title']=addslashes($title);
$_GET['touser']=$touser;
$_GET['fromuser']=$fromuser;
$_GET['content']=addslashes($title);
$_GET['mobile']=$mobile;
$_GET['truename']=$truename;
$_GET['email']=$email;
$_GET['linkurl']=$linkurl;
if(!empty($mobile)){
$db->query("UPDATE {$AJ_PRE}member SET message=message+1 WHERE username='$touser'");
...
$housenums = $db->count($biao, $condition);
$result = $db->query("SELECT * FROM $biao WHERE $condition ORDER BY itemid DESC LIMIT $offset,$pagesize");
while($r = $db->fetch_array($result)) {
	$tags[] = $r;
		}
...
$title = unicode_encode($v['title']);
$address = unicode_encode($v['address']);
$lp_edu = unicode_encode($v['lp_edu']);
$price=$v['price'] ? $v['price'].'元/㎡' : '待定';
$saleStates=$_saleState[$v['typeid']];
$saleState = unicode_encode(getbox_diaoval('buildtype','checkbox',$v[buildtype],'newhouse_6'));
$r = array('projectId'=>$v['itemid'],'projectName'=>$title,'areaName'=>area_pos($v[areaid], ''),'effectPhoto'=>$v['thumb'],'averagePrice'=>$price,'propertyType'=>$v['catid'],'longitude'=>$lngX,'latitude'=>$latY,'juniorSchool'=>$lp_edu,'projectFeature'=>$saleState,'saleState'=>$saleStates,'haveCommission'=>$v['isfx'],'groupDiscount'=>$v['areaid'],'houseType'=>search_cats($v[catid], '6'));
$json[] = JSON($r);
...
$json_str ="{\"code\":\"101\",\"msg\":\"success\",\"data\":{";
$json_str .= "\"list\":[";
$json_str .= implode(',',$json);
$json_str .= "],\"count\":\"".$housenums."\"}}";
echo $callback.$json_str;
}
else
{
...
$_POST['linkurl']=$linkurl;
if(!empty($telephone)){
$db->query("UPDATE {$AJ_PRE}group_15 SET orders=orders+1 WHERE itemid=$itemid");
$db->query("INSERT INTO {$AJ_PRE}group_order_15 (gid,buyer,seller,title,thumb,price,number,amount,logistic,addtime,updatetime,note, buyer_postcode,buyer_address,buyer_name,buyer_phone,buyer_mobile,status) VALUES ('$itemid','$touser','$fromuser','$title','$item[thumb]','$item[price]','$number','$amount','$item[logistic]','$AJ_TIME','$AJ_TIME','$content','$buyer_postcode','$buyer_address','$truename','$telephone','$telephone', 6)");
