...
function im_web($id, $style = 0) {
	global $MODULE, $AJ_PC;
	return $id ? '<a href="'.($AJ_PC ? $MODULE[2]['linkurl'] : $MODULE[2]['mobile']).'chat.php?touser='.$id.'" target="_blank" rel="nofollow"><img src="'.AJ_PATH.'api/online.png.php?username='.$id.'&style='.$style.'" title="点击交谈/留言" alt="" align="absmiddle" onerror="this.src=DTPath+\'file/image/web-off.gif\';"/></a>' : '';
}
...
function im_wx($id, $username = '', $style = 0) {
	global $AJ_PC;
	return $id ? '<a href="'.($AJ_PC ? AJ_PATH : AJ_MOB).'api/wx.php?wid='.$id.'&username='.$username.'" target="_blank" rel="nofollow"><img src="'.AJ_PATH.'file/image/wx.png" title="点击微信联系" alt="" align="absmiddle"/></a>' : '';
}
