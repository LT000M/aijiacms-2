define(function(require, exports, module) {
	...
	return function(opt){
		...
		$(opt.elm).on("click",function(){
			var $t=$(this),
				html='<div id="FCYY"><div class="yiyuyue">您要团购看房的楼盘是：'+$t.data("info")+'</p></div><form action=""><input type="hidden" name="sex" value="先生"/><input type="hidden" name="itemid" value="'+$t.data("val")+'"/><input type="hidden" name="title" value="'+$t.data("info")+'"/><input type="hidden" name="areaid" value="'+$t.data("areaid")+'"/><input type="hidden" name="linkurl" value="'+$t.data("url")+'"/><input type="hidden" name="touser" value="'+$t.data("touser")+'"/><input type="hidden" name="fromuser" value="'+opt.fromuser+'"/><input type="hidden" name="housetype" value="'+$t.data("housetype")+'"/><h2></em></h2><ul><li><span> 联 系 人： </span><input name="truename" id="fcyyun" value="'+opt.truename+'" type="text" class="txt"';
			if(opt.name.length)
				html+='readonly>';
			...
			alertM(html,{
				...
				yf:function(){
					...
					else if(!/^1[3456789]\d{9}$/.test($m.val())){
						$i.html("请填写正确的手机号码")
						return false;
					}
					...
					$.ajax({
						...
						if(data.state=="succ"){
							var html='<div class="FC"><div class="okh"><div class="big succ"><b>团购看房提交成功！</b><p>稍后我将在线查看您的看房申请，<br> 我会尽快与您联系，请耐心等待~</p>';
							if(data.url)
								html+='<i id="FCbtn2"></i>';
