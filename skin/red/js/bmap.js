	...
	return {
			...
			if (searchType == "xinfang") {
				...
				$.ajax({
					...
					if (data != null) {
						...
						room+="</ul>";
						var html='<div class="cf" id="esfalert"><div class="fl">'+price+room+'<ul><li><a target="_blank" class="map2_comm_moreinfo" href="'+COMMUNITY_URL+data.communityid+'/">查看小区详情&gt;&gt;</a></li><li><a target="_blank" class="map2_comm_fresh" href="'+COMMUNITY_URL+data.communityid+'/price.html">查看小区房价&gt;&gt;</a></li></ul></div><div id="map2_propertys"><div id="map2_info_sort"><span class="fr">排序：<a rel="area" href="javascript:;" class="lightbtn">按建面排序</a><a rel="price" href="javascript:;" class="lightbtn">按价格排序</a></span><span class="span_propnum"> 找到 <em id="house_nums" class="prop_num" style="">'+data.nums+'</em> 套房子 </span></div>';
						var list="";
						if(data.nums-0>0){
							...
							of:function(){
								...
								$('#map2_info_sort').on('click','.lightbtn',function(){
									if($(this).attr('rel') == 'area'){
										...
										$(this).addClass('on').prev().text("按建面排序").removeClass('on');
		...
		initBmap: function(params) {
			...
			if (searchType == "xinfang") {
				...
				if (!$.isEmptyObject(params)) {
					...
					if (params.f3 != undefined && parseInt(params.f3) >= 0) {
						filter.area = parseInt(params.f3); //建面
					} else {
						params.f3 = '';
						filter.area = 0;
					}
		...
		initSearchStyle: function(params) {
			...
			if (searchType == "xinfang") {
				...
				$.each($('#filter_area a'), function() { //建面
