	...
	var URL = "";
			...
			var mod = module.exports,
				...
				$buildarea = $("#buildarea").on("focus", function() {
					...
					if ($buildarea.val() == "请输入建筑建面") $buildarea.val("")

				}).on("blur",function(){

					var t = $buildarea.val();

					if (!t.length) $buildarea.val("请输入建筑建面");
			...
			if (formtype == "esf" || formtype == "zf" || formtype=="xzl") {
				...
				var delay, rsDelay,
					...
					$pop = $("#autoVn").on("mouseenter", "li", function() {
						...
						areaid_1 = $(this).find("strong").text(),

						map = $(this).find("em").text();
						...
						keyup: function(e) {
							...
							switch (e.which) {
								...
								default:
										...
										$.ajax({
											...
											if (data.length > 0) {
												...
												for (; i < l; i++) {
													...
													html += '<li data-val="'+data[i].hid+'" data-area="'+data[i].areaid+'"><b>' + data[i].name + '</b><span> ' + data[i].address + '</span><em style="display:none;"> ' + data[i].map + '</em><strong style="display:none;"> ' + data[i].areaid + '</strong></li>';
									...
									$pop.html('<ul></ul><p>未找到所需'+vsname+'？请直接输入小区名称 <a href=javascript:window.close()" class="red"><b>关闭'+vsname+'</b></a></p>');
