define(function(require, exports, module) {
	...
	return {
		...
		init: function(opt) {
			...
			var $ba_info = $("#ba_info"),
				...
				$buildarea = $("#buildarea").on("blur", function() {
					...
					if (/^[1-9]{1}(([0-9]{0,7})|([0-9]{0,4}\.{1}[0-9]{1,2})|([0-9]{0,5}\.{1}[0-9]?))$/.test(t)) {
						...
						mod.area = 0;
						$buildarea.parent().find("span").last().tip(0, "建面限最大8位数字，小数点后最多两位");
					}
				}),
				$room = $("#room"),
				$halls = $("#hall"),
				...
				$mobile = $("#mobile").on("blur", function() {
					if (/^[0-9]*$|^0\d{2,3}\d{7,8}?$/.test($mobile.val())) {
						$.ajax({
							url: opt.mobile_check,
							type: "post",
							dataType: 'json',
							data: {
								mobile: $mobile.val()
							...
							$password.parent().find("span").last().tip().remove();
							$mobile_code.parent().find("span").last().tip().remove();
							if (data.state == "succ") {
								mod.mobile_type = data.mobiletype;
								$pwd_need.val(mod.mobile_type);
								if (mod.mobile_type != 3) {
									if (mod.mobile_type == 1) {
										$mobile_code_li.show();
										$password_li.hide();
									} else {
										$mobile_code_li.hide();
				...
				$mobile_code = $("#mobile_code").on("blur", function() {
					if ($mobile_code.val().length < 6) {
						$mobile_code.parent().find("span").last().tip(0, "验证码不得少于6位")
					} else {
						$mobile_code.parent().find("span").last().tip().remove();
					}
				}),
				$send = $("#mobile_code_send").on("click", function() {
					if (/^[0-9]*$|^0\d{2,3}\d{7,8}?$/.test($mobile.val())) {
						$send.hide();
						$wait.show().html("正在发送，请稍候…<i></i>");
						$.ajax({
							url: opt.code_send,
							type: "post",
							dataType: 'json',
							...
							if (data.state == "succ") {
								delay = setInterval(mobileTime, 999);
							} else {
								$send.show();
								$wait.hide();
							}
				...
				$wait = $("#mobile_code_wait"),
				$pwd_need=$("#pwd_need"),
				$password = $("#password").on("blur", function() {
					if ($password.val().length < 6) {
						$password.parent().find("span").last().tip(0, "密码不得少于6位")
					} else {
						$password.parent().find("span").last().tip().remove();
					}
				}),
				$forget_pwd = $("#forget_pwd").on("click", function() {
					mod.mobile_type = 1;
					$pwd_need.val(mod.mobile_type);
					$mobile_code_li.show();
					$password_li.hide().parent().find("span").last().tip().remove();
				}),
				$mobile_code_li = $("#mobile_code_li"),
				$password_li = $("#password_li,#password_li2"),
				time = 121,
				delay = 0,
				mobileTime = function() {
					if (!--time) {
						$send.html("重新获取验证码").show();
						$wait.hide();
						time = 121;
						clearInterval(delay);
					} else $wait.html(time + "秒后重发验证码<i></i>");
				},
				user_check = function() {
					var b = 1;
					...
					if (mod.mobile_type == 1 && !$mobile_code.val().length) {
						b = 0;
						$mobile_code.trigger("blur");
					}
					if (mod.mobile_type == 2 && !$password.val().length) {
						b = 0;
			...
			window.setEsf = function(name, id) {
				alertM.remove();
				mod.esf = 1;
				mod.esf_name = name;
				$esf_cid.val(id);
