...
$url = isset($url) ? fix_link($url) : AJ_MOB;
if(isset($username)) {
	...
	if($aid) {
		...
		if($r) {
			...
			$db->query("UPDATE LOW_PRIORITY {$AJ_PRE}ad SET hits=hits+1 WHERE aid=$aid", 'UNBUFFERED');
		}
	}
} else if($mid) {
	if(isset($MODULE[$mid]) && !$MODULE[$mid]['islink'] && $itemid && $mid > 3) {
		...
		$table = get_table($mid);
		if($MODULE[$mid]['module'] == 'job' && $page == 2) $table = $AJ_PRE.'job_resume_'.$mid;
		...
		if($itemid && in_array($tb, array('announce', 'webpage', 'link', 'gift', 'vote', 'poll', 'form'))) {
			$r = $db->get_one("SELECT * FROM {$AJ_PRE}{$tb} WHERE itemid=$itemid");			
			if($r && $r['linkurl']) {
				if($tb == 'webpage') {
					$url = strpos($r['linkurl'], '://') === false ? AJ_MOB.$r['linkurl'] : $r['linkurl'];
				} else {
					$k = $tb.'_mob';
