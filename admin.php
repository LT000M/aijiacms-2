...
$_catid = $_child = array();
if($action != 'login') {
	if($_groupid != 1 || $_admin < 1 || !$_aijiacms_admin) msg('', '?action=login&forward='.urlencode($AJ_URL));
	if(!admin_check()) {
		admin_log(1);
...
if($module == 'aijiacms') {
	@include AJ_ROOT.'/admin/'.$file.'.inc.php';
} else {
	include AJ_ROOT.'/module/'.$module.'/common.inc.php';
	@include AJ_ROOT.'/module/'.$module.'/admin/'.$file.'.inc.php';
}
?>