...
class upload {
    ...
    function __construct($_file, $savepath, $savename = '', $fileformat = '') {
		...
		$this->fileformat = $fileformat ? $fileformat : $AJ['uploadtype'];
		$this->maxsize = $AJ['uploadsize'] ? $AJ['uploadsize']*1024 : 2048*1024;
	...
	function save() {
        ...
        if(!is_writable(AJ_ROOT.'/'.$this->savepath)) return $this->_('Error(24)'.$L['upload_unwritable']);
		if(!is_uploaded_file($this->file)) return $this->_('Error(25)'.$L['upload_failed']);
		if(!move_uploaded_file($this->file, AJ_ROOT.'/'.$this->saveto)) return $this->_('Error(26)'.$L['upload_failed']);
		$this->image = $this->is_image();
		if(AJ_CHMOD) @chmod(AJ_ROOT.'/'.$this->saveto, AJ_CHMOD);
        return true;
	}
