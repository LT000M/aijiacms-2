...
function cache_module($moduleid = 0) {
	if(defined('AJ_MOB')) {
		...
		$r = DB::get_one("SELECT * FROM ".AJ_PRE."setting WHERE item_key='mobile_domain'");
		if($r && $r['item_value']) {
			$AJ_MOB = $r['item_value'];
		} else {
			$AJ_MOB = AJ_PATH.'mobile/';
		}
	}
	if($moduleid) {
		$r = DB::get_one("SELECT * FROM ".AJ_PRE."module WHERE disabled=0 AND moduleid='$moduleid'");
		$setting = array();
		...
		$setting['mobile'] = $r['mobile'] ? $r['mobile'] : $AJ_MOB.$r['moduledir'].'/';
		if($moduleid == 3) {
		...
		$result = DB::query("SELECT moduleid,module,name,moduledir,domain,linkurl,style,listorder,islink,ismenu,icon,isblank,logo FROM ".AJ_PRE."module WHERE disabled=0 ORDER by listorder asc,moduleid desc");
		$CACHE = array();
		$modules = array();
		while($r = DB::fetch_array($result)) {
			if(!$r['islink']) {
				...
				if($linkurl != $r['linkurl']) {
					$r['linkurl'] = $linkurl;
					DB::query("UPDATE ".AJ_PRE."module SET linkurl='$linkurl' WHERE moduleid='$r[moduleid]' ");
				}
				$r['mobile'] = $AJ_MOB.$r['moduledir'].'/';
...
function cache_area() {
	$data = array();
    $result = DB::query("SELECT areaid,areaname,parentid,arrparentid,child,arrchildid FROM ".AJ_PRE."area ORDER BY listorder,areaid");
    while($r = DB::fetch_array($result)) {
		$areaid = $r['areaid'];
        $data[$areaid] = $r;
    }
...
function cache_area_ditie() {
	$data = array();
    $result = DB::query("SELECT areaid,areaname,parentid,arrparentid,child,arrchildid FROM ".AJ_PRE."area_ditie ORDER BY listorder,areaid");
    while($r = DB::fetch_array($result)) {
		$areaid = $r['areaid'];
        $data[$areaid] = $r;
    }
	cache_write('area_ditie.php', $data);
}
...
function cache_navmenu() {
	$data = array();
    $result = DB::query("SELECT * FROM ".AJ_PRE."navmenu ORDER BY listorder,navid");
    while($r = DB::fetch_array($result)) {
		$navid = $r['navid'];
        $data[$navid] = $r;
    }
	cache_write('navmenu.php', $data);
}
...
function cache_category($moduleid = 0, $data = array()) {
	...
	if($moduleid) {
		if(!$data) {
			$result = DB::query("SELECT * FROM ".AJ_PRE."category WHERE moduleid='$moduleid' ORDER BY listorder,catid");
			while($r = DB::fetch_array($result)) {
				$data[$r['catid']] = $r;
			}
		}
...
function cache_pay() {
	...
	$result = DB::query("SELECT * FROM ".AJ_PRE."setting WHERE item LIKE '%pay-%'");
	while($r = DB::fetch_array($result)) {
...
function cache_oauth() {
	...
	$result = DB::query("SELECT * FROM ".AJ_PRE."setting WHERE item LIKE '%oauth-%'");
	while($r = DB::fetch_array($result)) {
...
function cache_fields($tb = '') {
	if($tb) {
		$data = array();
		$result = DB::query("SELECT * FROM ".AJ_PRE."fields WHERE tb='$tb' ORDER BY listorder,itemid");
		while($r = DB::fetch_array($result)) {
			$data[$r['itemid']] = $r;
		}
		...
		$tbs = array();
		$result = DB::query("SELECT * FROM ".AJ_PRE."fields");
		while($r = DB::fetch_array($result)) {
...
function cache_group() {
	$data = $group = array();
	  //$type = isset($type);
	$result = DB::query("SELECT * FROM ".AJ_PRE."member_group ORDER BY listorder ASC,groupid ASC");
	while($r = DB::fetch_array($result)) {
		$groupid = $r['groupid'];
		$tmp = array();
...
function cache_type($item = '') {
	if($item) {
		$types = array();
		$result = DB::query("SELECT typeid,parentid,typename,style FROM ".AJ_PRE."type WHERE item='$item' AND cache=1 ORDER BY listorder ASC,typeid DESC");
		while($r = DB::fetch_array($result)) {
			$types[$r['typeid']] = $r;
		}
		...
		$arr = array();
		$result = DB::query("SELECT item FROM ".AJ_PRE."type WHERE item!='' AND cache=1 ORDER BY typeid DESC");
		while($r = DB::fetch_array($result)) {
...
function cache_bancomment($moduleid = 0) {
	...
	if($moduleid) {
		$data = array();
		$result = DB::query("SELECT itemid FROM ".AJ_PRE."comment_ban WHERE moduleid='$moduleid' ORDER BY bid DESC ");
		while($r = DB::fetch_array($result)) {
			$data[] = $r['itemid'];
		}
...
function cache_keylink($item = '') {
	if($item) {
		$keylinks = array();
		$result = DB::query("SELECT title,url FROM ".AJ_PRE."keylink WHERE item='$item' ORDER BY listorder DESC,itemid DESC");
		while($r = DB::fetch_array($result)) {
			$keylinks[] = $r;
		}
		...
		$arr = array();
		$result = DB::query("SELECT item FROM ".AJ_PRE."keylink");
		while($r = DB::fetch_array($result)) {
			if(!in_array($r['item'], $arr)) {
				$arr[] = $r['item'];
...
function cache_banip() {
	$data = array();
	$result = DB::query("SELECT ip,totime FROM ".AJ_PRE."banip ORDER BY itemid DESC");
	while($r = DB::fetch_array($result)) {
...
function cache_banword() {
	$data = array();
	$result = DB::query("SELECT * FROM ".AJ_PRE."banword ORDER BY bid DESC");
	while($r = DB::fetch_array($result)) {
		$b = array();
		$b[] = $r['replacefrom'];
		$b[] = $r['replaceto'];
