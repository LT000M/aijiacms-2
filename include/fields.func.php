	...
	if($v['html'] == 'hidden') {
		...
		if($AJ_PC) {
			...
			if($v['input_limit']) {
				...
				$html .= defined('AJ_ADMIN') ? '<span class="f_hid">*</span> ' : '';
			}
			$html .= $v['title'];
			$html .= '</td>';
			$html .= $right;
		...
		switch($v['html']) {
			...
			case 'checkbox':
				if($v['option_value']) {
					$html .= '<span id="'.$v['name'].'">';
					    //增加全选
    $html .= ' <input type="button" value="全选" onclick=\'checkqx("post_fields['.$v['name'].'][]")\'/>&nbsp;&nbsp;';
					$value = explode(',', $value);
					$rows = explode("*", $v['option_value']);
					foreach($rows as $rw => $row) {
						if($row) {
...
function fields_htmltwo($left = '<td class="tl">', $right = '<td>', $values = array(), $fd = array()) {
	extract($GLOBALS, EXTR_SKIP);
	if($fd) $FD = $fd;
	$html = '';
	foreach($FD as $k=>$v) {
		if(!$v['display']) continue;
		if(!defined('AJ_ADMIN') && !$v['front']) continue;
		$html .= fields_showtwo($k, $left, $right, $values, $fd);
	}
	return $html;
}
function fields_showtwo($itemid, $left = 'class="inl">', $right = '</li>', $values = array(), $fd = array()) {
	extract($GLOBALS, EXTR_SKIP);
	if($fd) $FD = $fd;
	if(!$values) {
		if(isset($item)) $values = $item;
	...
	if($v['html'] == 'hidden') {
		...
		switch($v['html']) {
			...
			break;
			case 'editor':
				$toolbar = isset($GLOBALS['group_editor']) ? $GLOBALS['group_editor'] : 'Aijiacms';
