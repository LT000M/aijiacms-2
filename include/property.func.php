...
function property_update($post_ppt, $moduleid, $catid, $itemid) {
	...
	DB::query("DELETE FROM ".AJ_PRE."category_value WHERE moduleid=$moduleid AND itemid=$itemid");
	$ppt = array();
	foreach($OP as $v) {
	...
	foreach($post_ppt as $k=>$v) {
		...
		DB::query("INSERT INTO ".AJ_PRE."category_value (oid,moduleid,itemid,value) VALUES ('$k','$moduleid','$itemid','$v')");
	}
...
function property_check($post_ppt) {
	...
	foreach($OP as $v) {
		if($v['required'] && !$post_ppt[$v['oid']]) {
			...
			defined('AJ_ADMIN') ? msg($msg) : dalert($msg);
		}
	}
}
...
function property_option($catid) {
	$catid = intval($catid);
	$lists = array();
	$result = DB::query("SELECT * FROM ".AJ_PRE."category_option WHERE catid=$catid ORDER BY listorder ASC,oid ASC");
...
function property_value($moduleid, $itemid) {
	$lists = array();
	$result = DB::query("SELECT oid,value FROM ".AJ_PRE."category_value WHERE moduleid=$moduleid AND itemid=$itemid");
	while($r = DB::fetch_array($result)) {
		$lists[$r['oid']] = $r['value'];
	}
	return $lists;
...
function property_condition($catid) {
	$catid = intval($catid);
	$lists = array();
	$result = DB::query("SELECT * FROM ".AJ_PRE."category_option WHERE catid=$catid AND type>1 AND search>0 ORDER BY listorder ASC,oid ASC");
	while($r = DB::fetch_array($result)) {
...
function property_html($var, $oid, $type, $value, $extend = '') {
	...
	if($AJ_PC) $str .= ' <span id="dproperty-'.$oid.'" class="f_red"></span>';
	return $str;
}
?>