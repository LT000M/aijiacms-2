...
class captcha {
	...
	function question($id) {
		$r = DB::get_one("SELECT * FROM ".AJ_PRE."question ORDER BY rand()");
		$_SESSION['answerstr'] = encrypt($r['answer'], AJ_KEY.'ANS');
	...
	function image() {
		...
		$string = $this->mk_str();
		$_SESSION['captchastr'] = encrypt(strtoupper($string), AJ_KEY.'CPC');
		...
		if($this->cn) {
			...
			$fonts = glob(AJ_ROOT.'/file/captcha/*.ttf');
			$num = count($fonts) - 1;
	...
	function mk_str() {
		$str = '';
		if($this->cn) {
			$step = AJ_CHARSET == 'UTF-8' ? 3 : 2;
			$text = substr(file_get(AJ_ROOT.'/file/config/cncaptcha.inc.php'), 13);
