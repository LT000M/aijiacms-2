...
function dhtmlspecialchars($string) {
	if(is_array($string)) {
		...
		$string = htmlspecialchars($string, ENT_QUOTES, AJ_CHARSET == 'GBK' ? 'GB2312' : 'UTF-8');
...
function strip_sql($string, $type = 1) {
	if(is_array($string)) {
		...
		if($type) {
			...
			$string = preg_replace_callback("/(select|update|replace|delete|drop)([\s\S]*?)(".AJ_PRE."|from)/i", 'strip_wd', $string);
			...
			$string = preg_replace_callback("/(union|where|having|outfile|dumpfile|".AJ_PRE.")/i", 'strip_wd', $string);
			return $string;
		} else {
...
function strip_uri($uri) {
	...
	if(strpos($uri, '<') !== false || strpos($uri, "'") !== false || strpos($uri, '"') !== false || strpos($uri, '0x') !== false) {
		dhttp(403, 0);
		dalert('HTTP 403 Forbidden - Bad URL', AJ_PATH);
	}
}
