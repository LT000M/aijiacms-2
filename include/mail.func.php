...
function dmail($mail_to, $mail_subject, $mail_body, $mail_from = '', $mail_sign = true) {
	...
	if($AJ['mail_type'] == 'sc') {
		...
		$par = 'api_user='.$AJ['smtp_user'].'&api_key='.$AJ['smtp_pass'].'&from='.$AJ['mail_sender'].'&fromname='.$AJ['mail_name'].'&to='.$mail_to.'&subject='.$mail_subject.'&html='.$mail_body;
		$rec = dcurl($url, $par);
		...
		$mail_from = $AJ['mail_name'] ? $AJ['mail_name'] : $AJ['sitename']."?= <".$sendmail_from.">";
