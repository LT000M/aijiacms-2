...
class remote {
	...
	function save() {
		...
		if(file_copy($this->file, AJ_ROOT.'/'.$this->saveto)) {
			if(!@getimagesize(AJ_ROOT.'/'.$this->saveto)) {
				file_del(AJ_ROOT.'/'.$this->saveto);
				return $this->_($L['upload_bad']);
			}
			if($this->maxsize > 0 && filesize(AJ_ROOT.'/'.$this->saveto) > $this->maxsize) {
				file_del(AJ_ROOT.'/'.$this->saveto);
