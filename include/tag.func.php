...
function tag($parameter, $expires = -1) {
	...
	if($catid) {
		if($moduleid > 4) {
			if(is_numeric($catid)) {
				$CAT = DB::get_one("SELECT child,arrchildid,moduleid FROM ".AJ_PRE."category WHERE catid=$catid");
				$condition .= ($child && $CAT['child'] && $CAT['moduleid'] == $moduleid) ? " AND catid IN (".$CAT['arrchildid'].")" : " AND catid=$catid";
			} else {
				if($child) {
					$catids = '';
					$result = DB::query("SELECT arrchildid FROM ".AJ_PRE."category WHERE catid IN ($catid)");
	...
	if($areaid) {
		if(is_numeric($areaid)) {
			$ARE = DB::get_one("SELECT child,arrchildid FROM ".AJ_PRE."area WHERE areaid=$areaid");
			...
			if($areachild) {
				$areaids = '';
				$result = DB::query("SELECT arrchildid FROM ".AJ_PRE."area WHERE areaid IN ($areaid)");
				while($r = DB::fetch_array($result)) {
	...
	while($r = DB::fetch_array($result)) {
		...
		if($lazy && isset($r['thumb']) && $r['thumb']) $r['thumb'] = AJ_SKIN.'image/lazy.gif" class="lazy" original="'.$r['thumb'];
		...
		if(isset($r['linkurl']) && $r['linkurl'] && $moduleid > 4 && strpos($r['linkurl'], '://') === false) $r['mobile'] = $MODULE[$moduleid]['mobile'].$r['linkurl'];
