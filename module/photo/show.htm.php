...
if($template == 'show-ebook' || $template == 'show-ebookfull') include AJ_ROOT.'/api/flashpageflip/make.inc.php';
...
for(; $page <= $total; $page++) {
	...
	$filename = AJ_ROOT.'/'.$MOD['moduledir'].'/'.itemurl($item, $page);
	...
	if($EXT['mobile_enable']) {
		if($total > 1) $pages = mobile_pages($total, $page, 1, $MOD['mobile'].itemurl($item, '{aijiacms_page}'));
		...
		file_put($filename, $data);
		if($page == 1 && $total > 1) file_copy($filename, str_replace(AJ_ROOT, AJ_ROOT.'/mobile', $indexname));
	}
}
return true;
?>