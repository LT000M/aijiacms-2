...
class spread {
    ...
    function __construct() {
		$this->table = AJ_PRE.'spread';
		$this->table_price = AJ_PRE.'spread_price';
	...
	function set($post) {
		...
		$m = DB::get_one("SELECT company FROM ".AJ_PRE."member WHERE username='$post[username]'");
	...
	function check($itemid, $status) {
		...
		if(is_array($itemid)) {
			...
			DB::query("UPDATE {$this->table} SET status=$status,editor='$_username',edittime=".AJ_TIME." WHERE itemid=$itemid");
			return true;
		}
	}
	...
	function _add($post) {
		...
		if($t) {
			$itemid = $t['itemid'];
			DB::query("UPDATE {$this->table_price} SET price='$price',edittime='".AJ_TIME."',editor='$_username' WHERE itemid='$itemid'");
		} else {
			DB::query("INSERT INTO {$this->table_price} (mid,word,price,editor,edittime) VALUES('$mid','$word','$price','$_username','".AJ_TIME."')");
		}
	}
	...
	function _edit($post) {
		foreach($post as $k=>$v) {
			...
			if($price > 0 && $price != dround($v['oldprice'])) DB::query("UPDATE {$this->table_price} SET price='$price',edittime='".AJ_TIME."',editor='$_username' WHERE itemid='$k'");
		}
	}
