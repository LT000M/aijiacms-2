...
class ad {
    ...
    function __construct() {
		$this->table = AJ_PRE.'ad';
		$this->table_place = AJ_PRE.'ad_place';
    }
	...
	function set_place($place) {
		...
		if(!$this->pid) $place['addtime'] = AJ_TIME;
		$place['edittime'] = AJ_TIME;
	...
	function delete_place($pid) {
		if(is_array($pid)) {
			...
			file_del(AJ_CACHE.'/htm/'.$filename);
			file_del(AJ_CACHE.'/htm/ad_'.$pid.'.htm');
			file_del(AJ_ROOT.'/file/script/A'.$pid.'.js');
	...
	function set_ad($ad) {
		...
		if(!$this->aid) $ad['addtime'] = AJ_TIME;
		$ad['edittime'] = AJ_TIME;
		$ad['editor'] = $_username;
