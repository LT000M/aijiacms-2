...
require AJ_ROOT.'/module/'.$module.'/gift.class.php';
$do = new gift();
$menus = array (
...
switch($action) {
	...
	case 'html':
		...
		if($fid <= $tid) {
			$result = $db->query("SELECT itemid,linkurl FROM {$AJ_PRE}gift WHERE itemid>=$fid ORDER BY itemid LIMIT 0,$num");
			if($db->affected_rows($result)) {
				while($r = $db->fetch_array($result)) {
					$itemid = $r['itemid'];
					$linkurl = $do->linkurl($itemid);
					if($linkurl != $r['linkurl']) $db->query("UPDATE {$AJ_PRE}gift SET linkurl='$linkurl' WHERE itemid=$itemid");
				}
				$itemid += 1;
			} else {
				$itemid = $fid + $num;
