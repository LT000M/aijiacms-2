...
require AJ_ROOT.'/module/'.$module.'/announce.class.php';
$do = new announce();
$menus = array (
...
switch($action) {
	...
	case 'html':
		...
		if($fid <= $tid) {
			$result = $db->query("SELECT itemid,linkurl,islink FROM {$AJ_PRE}announce WHERE itemid>=$fid ORDER BY itemid LIMIT 0,$num");
			if($db->affected_rows($result)) {
				while($r = $db->fetch_array($result)) {
					$itemid = $r['itemid'];
					if(!$r['islink']) {
						$linkurl = $do->linkurl($itemid);
						if($linkurl != $r['linkurl']) $db->query("UPDATE {$AJ_PRE}announce SET linkurl='$linkurl' WHERE itemid=$itemid");
					}
				}
				$itemid += 1;
			} else {
