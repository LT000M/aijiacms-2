			...
			if($post['filename']) {
				if($this->itemid) {
					...
					if($r['linkurl'] != $post['filepath'].$post['filename']) {
						if(is_file(AJ_ROOT.'/'.$post['filepath'].$post['filename'])) return $this->_($L['webpage_pass_exist']);
					}
				} else {
					if(is_file(AJ_ROOT.'/'.$post['filepath'].$post['filename'])) return $this->_($L['webpage_pass_exist']);
				}
	...
	function add($post) {
		...
		if(!$post['islink']) {
			if($post['filename']) {
				...
				$linkurl = $post['filepath'].$this->itemid.'.'.$AJ['file_ext'];
			}
