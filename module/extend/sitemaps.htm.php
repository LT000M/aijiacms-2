...
$data .= '</urlset>';
$data = str_replace('><', ">\n<", $data);
file_put(AJ_ROOT.'/sitemaps.xml', $data);
foreach($mods as $mid) {
	if(isset($MODULE[$mid]) && !$MODULE[$mid]['islink'] && $MODULE[$mid]['domain']) {
		if($mid == 4 && $CFG['com_domain']) continue;
		$url = $MODULE[$mid]['linkurl'];
		...
		$data .= '</urlset>';
		$data = str_replace('><', ">\n<", $data);
		file_put(AJ_ROOT.'/'.$MODULE[$mid]['moduledir'].'/sitemaps.xml', $data);
	}
}
return true;
?>