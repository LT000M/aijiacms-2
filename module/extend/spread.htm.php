...
$item = $db->get_one("SELECT * FROM {$AJ_PRE}spread WHERE itemid=$itemid");
if(!$item) return false;

$filename = AJ_CACHE.'/htm/m'.urlencode($item['mid']).'_k'.urlencode($item['word']).'.htm';
if($AJ_TIME - @filemtime($filename) < 60) return false;

$result = $db->query("SELECT * FROM {$AJ_PRE}spread WHERE mid=$item[mid] AND word='$item[word]' AND fromtime<=$AJ_TIME AND totime>=$AJ_TIME ORDER BY price DESC,itemid ASC");
$totime = 0;
$itemids = array();
