...
class group {
    ...
    function __construct($moduleid) {
		...
		$this->fields = array('catid','areaid','level','title','style','fee','introduce','marketprice','price','savemoney','discount','minamount','amount','logistic','thumb','tag','status','hits','orders','username','totime','editor','addtime','edittime','ip','template','linkurl','filepath','note','company','truename','telephone','mobile','address','email','qq','wx','ali','skype','houseid','housename','endtime','process');
    }
	...
	function set($post) {
		...
		$post['endtime'] = is_time($post['endtime']) ? strtotime($post['endtime']) : 0;
	...
	function expire($condition = '') {
		DB::query("UPDATE {$this->table} SET status=4 WHERE status=3 AND totime>0 AND totime<".AJ_TIME." $condition");
	}
	...
	function refresh($itemid) {
		$itemids = is_array($itemid) ? implode(',', $itemid) : $itemid;
		DB::query("UPDATE {$this->table} SET edittime='".AJ_TIME."' WHERE itemid IN ($itemids)");
	}
