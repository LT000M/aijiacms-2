...
if($action == 'refund' || $action == 'show') {
	...
	$td or msg('记录不存在');
	$td['linkurl'] = AJ_PATH.'api/redirect.php?mid=17&itemid='.$td['itemid'];
	$td['money'] = $td['amount'];
...
switch($action) {
	...
	case 'refund':
		...
		if($submit) {
			...
			$db->query("UPDATE {$table} SET status=$status,editor='$_username',updatetime=$AJ_TIME,refund_reason='$content' WHERE itemid=$itemid");
			$msg = isset($msg) ? 1 : 0;
			$eml = isset($eml) ? 1 : 0;
			$sms = isset($sms) ? 1 : 0;
			$wec = isset($wec) ? 1 : 0;
	...
	default:
		...
		while($r = $db->fetch_array($result)) {
			...
			$r['linkurl'] = AJ_PATH.'api/redirect.php?mid='.$moduleid.'&itemid='.$r['gid'];
