...
if($submit) {
	...
	$db->query("INSERT INTO {$table_order} (gid,buyer,seller,title,thumb,price,number,amount,logistic,addtime,updatetime,note, buyer_postcode,buyer_address,buyer_name,buyer_mobile,status) VALUES ('$itemid','$_username','$item[username]','$title','$item[thumb]','$item[price]','$number','$amount','$item[logistic]','$AJ_TIME','$AJ_TIME','$note','$buyer_postcode','$buyer_address','$buyer_name','$buyer_mobile', 6)");
	$oid = $db->insert_id();
	dheader('?action=show&itemid='.$itemid.'&auth='.encrypt('mid='.$moduleid.'&action=update&step=pay&itemid='.$oid, AJ_KEY.'TURL'));
}
$CSS = array('cart');
$head_title = $L['buy_title'];
if($action == 'show') {
	$forward = isset($auth) ? decrypt($auth, AJ_KEY.'TURL') : '';
	$forward = ($AJ_PC ? $MODULE[2]['linkurl'] : $MODULE[2]['mobile']).'deal.php?'.($forward ? $forward : 'action=order');
} else {
