...
$mod_limit > -1 or dheader(($AJ_PC ? $MODULE[2]['linkurl'] : $MODULE[2]['mobile']).'account.php?action=group&itemid=1');
...
switch($action) {
	case 'add':
		...
		$need_password = $fee_add && $fee_currency == 'money' && $fee_add > $AJ['quick_pay'];
		...
		if($submit) {
			...
			if($do->pass($post)) {
				...
				if($MOD['upload_thumb'] && $MG['upload'] && strlen($post['thumbs']) < 5) dalert($L['sell_upload_image']);
			...
			if($itemid) {
				$MG['copy'] && $_userid or dheader(($AJ_PC ? $MODULE[2]['linkurl'] : $MODULE[2]['mobile']).'account.php?action=group&itemid=1');
				$do->itemid = $itemid;
				$item = $do->get_one();
				if(!$item || $item['username'] != $_username) message();
				extract($item);
				$totime = $totime > $AJ_TIME ? timetodate($totime, 6) : '';
			} else {
				$_catid = $catid;
	...
	break;
	case 'refresh':
		$MG['refresh_limit'] > -1 or dheader(($AJ_PC ? $MODULE[2]['linkurl'] : $MODULE[2]['mobile']).'account.php?action=group&itemid=1');
