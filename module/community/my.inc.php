	...
	case 'add':
		...
		if($submit) {
			...
			if($do->pass($post)) {
				...
				if($AJ['mail_fabu']){
	               $linkurl = $MOD['linkurl'].'show.php?itemid='.$do->itemid;
					$subject = '你好，我发布了一条新房'.$post['title'];
					$content = '姓名：'.$post['truename'].'联系电话:'.$post['mobile'].'<br/>';
					$content .= '请点击下面的链接查看详情：<br/>';
					$content .= '<a href="'.$linkurl.'" target="_blank">'.$post['title'].'</a><br/>';
					$user = userinfo($a['username']);
					$email=$AJ['mail_sender'];
					 send_mail($email, $subject, $content,'email-default');
