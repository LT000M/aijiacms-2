...
class special {
    ...
    function __construct($moduleid) {
		...
		$this->fields = array('catid','areaid','level','title','style','introduce','thumb','banner','cfg_photo','cfg_video','cfg_type','seo_title','seo_keywords','seo_description','tag','status','hits','username','addtime', 'editor','edittime','ip','template','template_type','islink','linkurl','filepath','domain','note');
    }
	...
	function delete($itemid, $all = true) {
		...
		if(is_array($itemid)) {
			...
			if($all) {
				...
				DB::query("DELETE FROM ".AJ_PRE."special_item_".$this->moduleid." WHERE specialid=$itemid");
				DB::query("DELETE FROM ".AJ_PRE."type WHERE item='special-".$itemid."'");
				$content_table = content_table($this->moduleid, $this->itemid, $this->split, $this->table_data);
