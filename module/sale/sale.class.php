...
class sale {
    ...
    function __construct($moduleid) {
		...
		$this->fields = array('catid','mycatid','areaid','typeid','level','title','tag','style','fee','introduce','price','days','thumb','thumbs','video','thumb1','thumb2','thumb3','thumb4','tag','status','hits','username','totime','editor','addtime','adddate','edittime','editdate','ip','template','linkurl','filepath','elite','note','company','truename','telephone','mobile','address','email','msn','qq','wx','ali','skype','housename','houseid','room','hall','balcony','houseearm','floor1','floor2','cqxz','houseyear','map','toward','toilet','bus');
...
$status = get_status(3, $need_check);
       ...
       $borough_id=$this->houseid;
		...
		$post['map'] = $post['map'] ? $post['map']:$borough_map;
	...
	function get_list($condition = 'status=3', $order = 'edittime DESC', $cache = '') {
		...
		while($r = DB::fetch_array($result)) {
			$r['alt'] = $r['title'];
			$houseid = $r['itemid'];
			 $isappo = DB::get_one("SELECT * FROM ".AJ_PRE."appolist WHERE   house_id = $houseid AND appo_site='sale'");
	...
	function add($post) {
		...
		//20220803
				if($AJ['baidu_tui']) baidu($this->itemid,$this->table);//发布直接推送到百度
...
function saletop($itemid,$to_time) {
	       $sale=AJ_PRE.'sale_5';
			DB::query("UPDATE $sale SET istop=1,to_time=$to_time WHERE itemid=$itemid");
			return true;
          	}
	function salehot($itemid,$to_time) {
	       $sale=AJ_PRE.'sale_5';
			DB::query("UPDATE $sale SET ishot=1,hot_time=$to_time WHERE itemid=$itemid");
			return true;
     }
