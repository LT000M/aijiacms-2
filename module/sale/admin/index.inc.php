...
if(in_array($action, array('', 'check', 'expire', 'reject', 'recycle'))) {
	$sfields = array('模糊', '房源名' , '联系人', '联系电话', '联系地址', '电子邮件',  '会员名', 'IP');
	$dfields = array('keyword', 'title',  'truename', 'telephone', 'address', 'email', 'username', 'ip');
	$sorder  = array('结果排序方式', '更新时间降序', '更新时间升序', '添加时间降序', '添加时间升序', VIP.'级别降序', VIP.'级别升序', '价格降序', '价格升序', '浏览次数降序', '浏览次数升序', '信息ID降序', '信息ID升序', '建面降序', '建面升序');
	$dorder  = array($MOD['order'], 'edittime DESC', 'edittime ASC', 'addtime DESC', 'addtime ASC', 'vip DESC', 'vip ASC', 'price DESC', 'price ASC',  'hits DESC', 'hits ASC', 'itemid DESC', 'itemid ASC','houseearm DESC', 'houseearm ASC');
