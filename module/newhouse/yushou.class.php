...
class yushou {
 ...
 function __construct() {
		global $db,$pid,$title;
		$this->table = $db->pre.'newhouse_yushou';
		$this->db = &$db;
        $this->fields = array('title','houseid','introduce','thumb','listorder','catid','status','username','addtime','edittime','project','acreage','residence','noneresidence','card_time','areaid','note','kfs');
    }
 ...
 function yushou() {
	...
	function pass($post) {
		...
		if(!$post['title']) return $this->_($L['msg_yushou']);
		return true;
	}
	...
	function get_list($condition = '1', $order = 'addtime DESC') {
		global $pages, $page, $pagesize, $offset, $pagesize, $MOD, $item, $sum,$catids;
		if($page > 1 && $sum) {
			$item = $sum;
		} else {
