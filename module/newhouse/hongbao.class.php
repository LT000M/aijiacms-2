...
class hongbao {
 ...
 function __construct() {
	global $db,$pid,$title;
		$this->table = $db->pre.'hongbao';
		$this->table_product = $db->pre.'newhouse_6';
		$this->db = &$db;
		$this->fields = array('house','hname','money','username','truename','mobile','addtime','status','editor','edittime','note');
    }
 ...
 function hongbao() {
	...
	function pass($post) {
		...
		if(!$post['hongbao']) return $this->_($L['msg_hongbao']);
		return true;
	}
