...
$do = new xiangce;
$menus = array (
	array('添加相册', '?file='.$file.'&moduleid='.$moduleid.'&houseid='.$houseid.'&title='.$title.'&action=add'),
	array('相册管理', '?file='.$file.'&moduleid='.$moduleid.'&houseid='.$houseid.'&title='.$title),
	array('相册审核', '?file='.$file.'&moduleid='.$moduleid.'&houseid='.$houseid.'&action=check'),
...
switch($action) {
	...
	global $note;
		if($submit) {
			...
			include tpl('xiangce_add', $module);
		}
	break;
	case 'edit':
		$itemid or msg('请选择信息');
		...
		if($submit) {
			...
			$menuid = 1;
			include tpl('xiangce_edit', $module);
		}
	break;
	case 'delete':
		$itemid or msg('请选择信息');
