...
if(in_array($action, array('', 'check', 'expire', 'reject', 'recycle', 'fenxiao'))) {
	$sfields = array('模糊', '楼盘名称', '会员名');
	$dfields = array('keyword', 'title', 'username');
	$sorder  = array('结果排序方式', '更新时间降序', '更新时间升序', '添加时间降序', '添加时间升序', '开盘时间降序', '开盘时间升序', '交房时间降序', '交房时间升序', VIP.'级别降序', VIP.'级别升序', '价格降序', '价格升序', '浏览次数降序', '浏览次数升序', '信息ID降序', '信息ID升序');
	$dorder  = array($MOD['order'], 'edittime DESC', 'edittime ASC', 'addtime DESC', 'selltime ASC', 'selltime DESC', 'addtime ASC', 'completion DESC', 'completion ASC', 'vip DESC', 'vip ASC', 'price DESC', 'price ASC',  'hits DESC', 'hits ASC', 'itemid DESC', 'itemid ASC');
	...
	$type_select = dselect($TYPE, 'typeid', $MOD['name'].'状态', $typeid);
	...
	if($typeid >=0) $condition .= " AND typeid=$typeid+1";
...
switch($action) {
	...
	case 'edit':
		...
		if($submit) {
			...
			$hbtime = $totime ? timetodate($hbtime, 3) : '';
	...
	break;
	case 'recycle':
		$lists = $do->get_list('status=0 and isnew=1'.$condition, $dorder[$order]);
		$menuid = 4;
	...
	case 'reject':
		if($itemid && !$psize) {
			...
			$lists = $do->get_list('status=1 and isnew=1'.$condition, $dorder[$order]);
			$menuid = 3;
			include tpl('index', $module);
		}
	break;
	...
	case 'check':
		if($itemid && !$psize) {
			...
			$lists = $do->get_list('status=2 and isnew=1'.$condition, $dorder[$order]);
			$menuid = 2;
			include tpl('index', $module);
		}
	break;
