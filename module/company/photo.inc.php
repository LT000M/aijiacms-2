...
$table_item = $AJ_PRE.$module.'_item_'.$moduleid;
if($itemid) {
	...
	if($open < 3) {
		...
		$pass = $str == md5(md5($AJ_IP.$open.$_key.AJ_KEY));	
	...
	if($pass) {
		...
		if($view) {
			...
			$demo_url = userurl($username, 'file='.$file.'&itemid='.$itemid.'&view=1&page={aijiacms_page}', $domain).'#p';
			...
			$demo_url = userurl($username, "file=$file&itemid=$itemid&page=".'{aijiacms_page}', $domain);
		...
		if($submit) {
			if(isset($key) && $key == $_key) {
				$pass = true;
				set_cookie('photo_'.$itemid, md5(md5($AJ_IP.$open.$_key.AJ_KEY)), $AJ_TIME + 86400);
				dheader($linkurl);
			} else {
