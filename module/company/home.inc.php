...
switch($action) {
	case 'search':
		if(check_name($homepage) && in_array($file, array('sell', 'buy', 'news', 'credit', 'job', 'price', 'photo', 'info', 'brand', 'video', 'mall')) && $kw) {
			...
			$domain = $user['domain'];
			if($domain) $AJ['rewrite'] = intval($CFG['com_rewrite']);
	...
	case 'send':
		...
		if($job == 'inquiry') {
			if($MG['inquiry_limit']) {
				$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}message WHERE $sql AND addtime>$today AND typeid=1 AND status=3");
				$r['num'] < $MG['inquiry_limit'] or dalert(lang($L['message_limit'], array($MG['inquiry_limit'], $r['num'])));
			}
		} else if($job == 'price') {
			if($MG['price_limit']) {
				$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}message WHERE $sql AND addtime>$today AND typeid=2 AND status=3");
	...
	case 'next':
		...
		$domain = $user['domain'];
		if($domain) $AJ['rewrite'] = intval($CFG['com_rewrite']);
		$r = $db->get_one("SELECT itemid FROM {$AJ_PRE}sell_5 WHERE username='$username' AND itemid>$itemid AND status=3 ORDER BY itemid ASC");
	...
	case 'prev':
		...
		$domain = $user['domain'];
		if($domain) $AJ['rewrite'] = intval($CFG['com_rewrite']);
		$r = $db->get_one("SELECT itemid FROM {$AJ_PRE}sell_5 WHERE username='$username' AND itemid<$itemid AND status=3 ORDER BY itemid DESC");
		if($r) dheader(userurl($username, 'file=sell&itemid='.$r['itemid'], $domain));
		dheader(userurl($username, 'file=sell', $domain));
	break;
	default:
