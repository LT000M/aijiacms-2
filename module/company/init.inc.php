...
if($EXT['mobile_enable']) {
	if(AJ_TOUCH) include AJ_ROOT.'/include/mobile.inc.php';
	$head_mobile = $AJ_URL;
	$foot = '';
}
...
if($domain) {
	if(!isset($preview) && !isset($update) && !isset($key)) {
		if($CFG['com_domain']) {
			if(strpos($AJ_URL, $domain) === false) {
				$subdomain = userurl($username);
				if(strpos($AJ_URL, $subdomain) === false) {
					dheader('http://'.$domain.'/');
				} else {
					if($AJ_URL == $subdomain.'index.php' || $AJ_URL == $subdomain) dheader('http://'.$domain.'/');
					dheader(str_replace($subdomain, 'http://'.$domain.'/', $AJ_URL));
				}
			}
		} else {
...
if($MOD['delvip'] && $COM['vip'] && $COM['totime'] && $COM['totime'] < $AJ_TIME) {//VIP Expired
	$COM['vip'] = 0;
	...
	$db->query("UPDATE {$AJ_PRE}member SET groupid=$gid WHERE userid=$userid");
	$clean = 1;
}
if($COM['styletime'] && $COM['styletime'] < $AJ_TIME) {//SKIN Expired
	$COM['skin'] = $COM['template'] = '';
...
if($rewrite) {
	...
	$kw = isset($kw) ? strip_kw($kw, $AJ['max_kw']) : '';
...
if($COM['skin'] && $COM['template']) {
	...
	$r = $db->get_one("SELECT skin,template FROM {$AJ_PRE}style WHERE itemid=$COMGROUP[styleid]", 'CACHE');
	if($r) {
		$skin = $r['skin'];
		$template = $r['template'];
	}
...
if($file == 'homepage') {
	if($preview) {
		$preview = $db->get_one("SELECT * FROM {$AJ_PRE}style WHERE itemid={$preview}");
		if($preview) {
			$skin = $preview['skin'];
...
$HSPATH = AJ_STATIC.$MODULE[4]['moduledir'].'/skin/'.$skin.'/';
if(!$banner) $banner = is_file(AJ_ROOT.'/'.$MODULE[4]['moduledir'].'/skin/'.$skin.'/banner.jpg') ? $HSPATH.'banner.jpg' : '';
...
if($domain) {
	...
	if($CFG['com_domain']) {
		...
		$comment_proxy = AJ_PATH;
	}
}
$comment_proxy = encrypt($comment_proxy, AJ_KEY.'PROXY');
$album_js = 0;
