...
class company {
	...
	function get_vipt($username) {
		...
		$_year = $r['fromtime'] ? (date('Y', AJ_TIME) - date('Y', $r['fromtime']))*$MOD['vip_year'] : 0;
		$_year = $_year > $MOD['vip_maxyear'] ? $MOD['vip_maxyear'] : $_year;
		$m = DB::get_one("SELECT COUNT(*) AS num FROM ".AJ_PRE."honor WHERE username='$username' AND status=3");
