		...
		if($fid <= $tid) {
			...
			if($db->affected_rows($result)) {
				while($r = $db->fetch_array($result)) {
					...
					$db->query("UPDATE {$AJ_PRE}comment SET passport='$passport' WHERE username='$username'");
					$db->query("UPDATE {$AJ_PRE}know SET passport='$passport' WHERE username='$username'");
					$db->query("UPDATE {$AJ_PRE}know_answer SET passport='$passport' WHERE username='$username'");
					$db->query("UPDATE {$AJ_PRE}know_expert SET passport='$passport' WHERE username='$username'");
					$db->query("UPDATE {$AJ_PRE}know_vote SET passport='$passport' WHERE username='$username'");
					$db->query("UPDATE {$AJ_PRE}club SET passport='$passport' WHERE username='$username'");
					$db->query("UPDATE {$AJ_PRE}club_fans SET passport='$passport' WHERE username='$username'");
					$db->query("UPDATE {$AJ_PRE}club_group SET passport='$passport' WHERE username='$username'");
					$db->query("UPDATE {$AJ_PRE}club_reply SET passport='$passport' WHERE username='$username'");
					$db->query("UPDATE {$AJ_PRE}club SET replyer='$passport' WHERE replyuser='$username'");
				}
				$userid += 1;
			} else {
				$userid = $fid + $num;
