...
class article {
    ...
    function __construct($moduleid) {
		...
		$this->fields = array('catid','areaid','level','title','style','fee','subtitle','introduce','thumb','tag','author','copyfrom','fromurl','voteid','status','hits','username','addtime','editor','edittime','ip','template','islink','linkurl','filepath','note','houseid','housename');
    }
	...
	function set($post) {
		...
		if($post['addtime'] > AJ_TIME && $post['status'] == 3) $post['status'] = 4;
	...
	function delete($itemid, $all = true) {
		...
		if(is_array($itemid)) {
			...
			if($MOD['show_html'] && !$r['islink']) {
				...
				$i = 1;
				while($i) {
					$_file = AJ_ROOT.'/'.$MOD['moduledir'].'/'.itemurl($r, $i);
					if(is_file($_file)) {
						unlink($_file);
						$i++;
					} else {
