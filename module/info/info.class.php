...
class info {
    ...
    function __construct($moduleid) {
		...
		$this->fields = array('catid','level','title','style','fee','introduce','n1','n2','n3','v1','v2','v3','totime','areaid','thumb','thumb1','thumb2','status','hits','username','addtime','adddate','editor','edittime','editdate','ip','template','islink', 'linkurl','filepath','note','company','truename','telephone','mobile','address','email','qq','wx','ali','skype');
    }
	...
	function set($post) {
		global $MOD, $AREA, $_username, $_userid;
	...
	function refresh($itemid) {
		$editdate = timetodate(AJ_TIME, 3);
		$itemids = is_array($itemid) ? implode(',', $itemid) : $itemid;
		DB::query("UPDATE {$this->table} SET edittime='".AJ_TIME."',editdate='$editdate' WHERE itemid IN ($itemids)");
	}
