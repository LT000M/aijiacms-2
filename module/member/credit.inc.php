...
switch($action) {
		...
		if($MOD['credit_buy'] && $MOD['credit_price']) {
			...
			if($submit) {
				...
				if($amount > 0) {
					...
					money_record($_username, -$amount, $L['in_site'], 'system', $L['buy'].$AJ['credit_name'], $credit.$AJ['credit_unit']);
					if($credit > 0) {
						credit_add($_username, $credit);
						credit_record($_username, $credit, 'system', $L['buy'].$AJ['credit_name'], $amount.$AJ['money_unit']);
					}
				}
	...
	default:
		...
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}finance_credit WHERE $condition");
		$items = $r['num'];
		$pages = pages($items, $page, $pagesize);
		$lists = array();
		$result = $db->query("SELECT * FROM {$AJ_PRE}finance_credit WHERE $condition ORDER BY itemid DESC LIMIT $offset,$pagesize");
		$income = $expense = 0;
