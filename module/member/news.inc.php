...
$menu_id = 2;
require AJ_ROOT.'/module/'.$module.'/news.class.php';
$do = new news();
switch($action) {
	case 'add':
		...
		if($MG['hour_limit']) {
			$today = $AJ_TIME - 3600;
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}news WHERE username='$_username' AND addtime>$today");
		...
		if($MG['news_limit']) {
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}news WHERE username='$_username' AND status>0");
...
$limit_used = 0;
for($i = 1; $i < 4; $i++) {
	$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}news WHERE username='$_username' AND status=$i");
	$nums[$i] = $r['num'];
	$limit_used += $r['num'];
}
$nums[0] = count($TYPE);
