...
function _userinfo($mobile) {
	return DB::get_one("SELECT * FROM ".AJ_PRE."member m,".AJ_PRE."company c WHERE m.userid=c.userid AND m.mobile='$mobile'");
}
...
switch($action) {
	...
	case 'make':
		if(isset($make)) {
			...
			if($data) {
				$filename = timetodate($AJ_TIME, 'YmdHis').$title.'-'.$random.'-'.$page.'.txt';
				file_put(AJ_ROOT.'/file/mobile/'.$filename, trim($data));
				$page++;
		...
		if(isset($send)) {
			...
			if($sendtype == 1) {
				...
				if(is_mobile($mobile)) {
                 ...
                 $sms_code = ali_sms($mobile, $AJ['sms_code'], $data);
				 }else
				 {
