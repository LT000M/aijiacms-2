...
switch($action) {
	case 'edit':
		...
		if($submit) {
			...
			$days = 364;
			//echo timetodate('1540915200',3).'dsdsd';
			//$totime = timetodate($totime, 3);
			$totime = timetodate($totime, 3) ? timetodate($totime, 3) :timetodate($AJ_TIME + 86400*$days, 3);
...
class grade {
	...
	function edit($post) {
		...
		$gsql = $msql = $csql = '';
		$gsql = "edittime=".AJ_TIME.",editor='$_username',status=$post[status],message='$message',reason='$post[reason]',note='$post[note]'";
		if($post['status'] == 1) {
			//reject
			if($user) {
				if($item['amount']) {
			...
			if($user) {
				if($GROUP[$item['groupid']]['type']) {
					$t = DB::get_one("SELECT userid FROM ".AJ_PRE."company WHERE company='$post[company]' AND userid<>$user[userid]");
					if($t) msg('公司名称已存在');
				}
		...
		if($msql) DB::query("UPDATE ".AJ_PRE."member SET $msql WHERE userid=$item[userid]");
		if($csql) DB::query("UPDATE ".AJ_PRE."company SET $csql WHERE userid=$item[userid]");
