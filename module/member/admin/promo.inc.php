...
require AJ_ROOT.'/module/'.$module.'/promo.class.php';
$do = new promo();
$menus = array (
...
switch($action) {
	case 'add':
		if($submit) {
			...
			$fromtime = timetodate($AJ_TIME, 3).' 00:00:00';
			$menuid = 0;
	...
	case 'give':
		...
		if($submit) {
			...
			if($r['totime'] < $AJ_TIME) msg('优惠活动已结束');
			$title = addslashes($r['title']);
			$db->query("INSERT INTO {$AJ_PRE}finance_coupon (title,username,seller,addtime,fromtime,totime,price,cost,pid,editor,edittime,note) VALUES ('$title','$username','$r[username]','$AJ_TIME','$r[fromtime]','$r[totime]','$r[price]','$r[cost]','$itemid','$_username','$AJ_TIME','$note')");
	...
	case 'order':
		$itemid or msg();
		$r = $db->get_one("SELECT mid FROM {$AJ_PRE}order WHERE itemid=$itemid");
	...
	case 'coupon':
		...
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}finance_coupon WHERE $condition");
		$pages = pages($r['num'], $page, $pagesize);		
		$lists = array();
		$result = $db->query("SELECT * FROM {$AJ_PRE}finance_coupon WHERE $condition ORDER BY itemid DESC LIMIT $offset,$pagesize");
