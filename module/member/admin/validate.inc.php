...
switch($action) {
	case 'cancel':
		...
		foreach($itemid as $id) {
			...
			if($r) {
				...
				$db->query("UPDATE {$AJ_PRE}member SET `{$vfd}`=0 WHERE userid=$userid");
				...
				if($sms) send_sms($user['mobile'], $content.$AJ['sms_sign']);
	...
	case 'check':
		...
		foreach($itemid as $id) {
			...
			if($r) {
				...
				$fd = $r['type'];
				$vfd = 'v'.$r['type'];
				$db->query("UPDATE {$AJ_PRE}member SET `{$fd}`='$value',`{$vfd}`=1 WHERE userid=$userid");
				if($fd == 'company') $db->query("UPDATE {$AJ_PRE}company SET `company`='$value' WHERE userid=$userid");
	...
	case 'member':
		...
		if($page > 1 && $sum) {
			...
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}member_check WHERE $condition");
			$items = $r['num'];
		}
		$pages = pages($items, $page, $pagesize);	
		$lists = array();
		$result = $db->query("SELECT * FROM {$AJ_PRE}member_check WHERE $condition ORDER BY addtime DESC LIMIT $offset,$pagesize");
	...
	case 'show':
		...
		$t = $db->get_one("SELECT * FROM {$AJ_PRE}member_check WHERE username='$username'");
		$t or msg('记录不存在');
		$U = userinfo($username);
		$U or msg('会员不存在');
		...
		if($submit) {
			...
			if($sql1) $db->query("UPDATE {$AJ_PRE}member SET ".substr($sql1, 1)." WHERE userid=$userid");
			if($sql2) $db->query("UPDATE {$AJ_PRE}company SET ".substr($sql2, 1)." WHERE userid=$userid");
			if($sql3) $db->query("UPDATE {$content_table} SET ".substr($sql3, 1)." WHERE userid=$userid");
			$db->query("DELETE FROM {$AJ_PRE}member_check WHERE username='$username'");
