...
if(check_name($username)) {
	$U = $db->get_one("SELECT * FROM {$AJ_PRE}weixin_user WHERE username='$username'");
	$U or msg('用户未绑定微信帐号');
	$openid = $U['openid'];
} else if($openid) {
...
if($openid) {
	...
	$menus = array (
		...
		array('公众平台', AJ_PATH.'api/redirect.php?url=http://mp.weixin.qq.com/', 'target="_blank"'),
	);
}
switch($action) {
	case 'unbind':
		...
		$db->query("UPDATE {$AJ_PRE}weixin_user SET username='' WHERE itemid IN ($itemids)");
		dmsg('解除成功', $forward);
	break;
	case 'delete':
		$itemid or msg('请选择记录');
		$itemids = is_array($itemid) ? implode(',', $itemid) : $itemid;
		$db->query("DELETE FROM {$AJ_PRE}weixin_chat WHERE itemid IN ($itemids)");
		dmsg('删除成功', $forward);
	break;
	case 'edit':
		$itemid or msg('请选择记录');
		$user = $db->get_one("SELECT * FROM {$AJ_PRE}weixin_user WHERE itemid=$itemid");
		$user or msg('记录不存在');
		if($submit) {
			if(check_name($name)) {
				...
				$db->query("UPDATE {$AJ_PRE}weixin_user SET username='' WHERE username='$name'");
			}
			$db->query("UPDATE {$AJ_PRE}weixin_user SET username='$name' WHERE itemid=$itemid");
			dmsg('修改成功', '?moduleid='.$moduleid.'&file='.$file.'&action=user');
		} else {
	...
	case 'setting':
		if($submit) {		
			...
			$db->query("UPDATE {$AJ_PRE}setting SET item_value='$open' WHERE item_key='weixin' AND item='3'");
			cache_module(3);
	...
	case 'sync':
		...
		foreach($arr['data']['openid'] as $v) {
			$num++;
			$user = weixin_user($v);
			if(!$user) $db->query("INSERT INTO {$AJ_PRE}weixin_user (openid) VALUES ('$v')");
		}
	...
	case 'sync_user':
		require AJ_ROOT.'/api/weixin/init.inc.php';
		if(!isset($fid)) {
			$r = $db->get_one("SELECT min(itemid) AS fid FROM {$AJ_PRE}weixin_user");
			$fid = $r['fid'] ? $r['fid'] : 0;
		}
		isset($sid) or $sid = $fid;
		if(!isset($tid)) {
			$r = $db->get_one("SELECT max(itemid) AS tid FROM {$AJ_PRE}weixin_user");
			$tid = $r['tid'] ? $r['tid'] : 0;
		}
		isset($num) or $num = 50;
		if($fid <= $tid) {
			$result = $db->query("SELECT * FROM {$AJ_PRE}weixin_user WHERE itemid>=$fid ORDER BY itemid LIMIT 0,$num ");
			if($db->affected_rows($result)) {
				while($user = $db->fetch_array($result)) {
					...
					if($info) {
						if($info['subscribe'] == 0) {
							$sql = "subscribe=0,username='',edittime=$AJ_TIME";						
						} else {
							$sql = "subscribe=1,addtime=".$info['subscribe_time'].",edittime=$AJ_TIME";
	...
	case 'send':
		...
		if($str == 'http' && in_array($ext, array('jpg', 'amr', 'mp3', 'mp4'))) {
			if(strpos($word, AJ_PATH) === 0) {
				$file = str_replace(AJ_PATH, AJ_ROOT.'/', $word);
			} else {
				if($AJ['remote_url'] && strpos($word, $AJ['remote_url']) === 0) {
					$file = AJ_ROOT.'/file/temp/'.date('YmdHis', $AJ_TIME).mt_rand(10, 99).$_userid.'.'.$ext;
					file_copy($word, $file);
				}
			}
	...
	case 'load':
		...
		$i = $j = 0;
		if($chatlast) {
			$sql = "SELECT * FROM {$AJ_PRE}weixin_chat WHERE openid='$openid' AND event=0 AND addtime>$chatlast ORDER BY addtime DESC";
		} else {
			$sql = "SELECT * FROM {$AJ_PRE}weixin_chat WHERE openid='$openid' AND event=0 ORDER BY addtime DESC LIMIT 20";
		}
		$lists = array();
		$result = $db->query($sql);
	...
	case 'user':
		...
		$sfields = array('按条件', '会员名', '昵称', '城市', '省份', '国籍', '语言', 'OpenID');
		$dfields = array('username', 'username', 'nickname', 'city', 'province', 'country', 'language', 'openid');
		isset($fields) && isset($dfields[$fields]) or $fields = 0;
		$sorder  = array('结果排序方式', '关注时间降序', '关注时间升序', '打开时间降序', '打开时间升序', '登录时间降序', '登录时间升序', '登录次数降序', '登录次数升序');
		$dorder  = array('itemid DESC', 'addtime DESC', 'addtime ASC', 'visittime DESC', 'visittime ASC', 'logintime DESC', 'logintime ASC', 'logintimes DESC', 'logintimes ASC');
		...
		isset($datetype) && in_array($datetype, array('addtime', 'visittime', 'logintime')) or $datetype = 'addtime';
		...
		if($page > 1 && $sum) {
			...
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}weixin_user WHERE $condition");
			$items = $r['num'];
		}
		$pages = pages($items, $page, $pagesize);
		$lists = array();
		$result = $db->query("SELECT * FROM {$AJ_PRE}weixin_user WHERE $condition ORDER BY $dorder[$order] LIMIT $offset,$pagesize");
		while($r = $db->fetch_array($result)) {
			$r['adddate'] = timetodate($r['addtime'], 5);
			$r['visitdate'] = $r['visittime'] ? timetodate($r['visittime'], 5) : 'N/A';
	...
	case 'auto':
		if($submit) {		
				...
				if($k == 0) {
					if($v['keyword'] && $v['reply']) {
						...
						foreach($K as $i=>$W) {
							$keyword = trim($W);
							$reply = trim($R[$i]);							
							if($keyword && $reply) $db->query("INSERT INTO {$AJ_PRE}weixin_auto SET keyword='$keyword',reply='$reply'");
						}
					}
				} else {
					if(isset($v['delete'])) {
						$db->query("DELETE FROM {$AJ_PRE}weixin_auto WHERE itemid=$k");
					} else {
						$keyword = trim(strip_tags($v['keyword']));
						$reply = trim(strip_tags($v['reply']));
						if($keyword && $reply) $db->query("UPDATE {$AJ_PRE}weixin_auto SET keyword='$keyword',reply='$reply' WHERE itemid=$k");
					}
				}
			}
			dmsg('保存成功', '?moduleid='.$moduleid.'&file='.$file.'&action='.$action);
		} else {
			$condition = "1";
			...
			if($page > 1 && $sum) {
				...
				$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}weixin_auto WHERE $condition");
				$items = $r['num'];
			}
			$pages = pages($items, $page, $pagesize);
			$lists = array();
			$result = $db->query("SELECT * FROM {$AJ_PRE}weixin_auto WHERE $condition ORDER BY itemid LIMIT $offset,$pagesize");
			while($r = $db->fetch_array($result)) {
				$lists[] = $r;
			}
	...
	default:
		...
		if($fromtime) $condition .= " AND c.addtime>=$fromtime";
		if($totime) $condition .= " AND c.addtime<=$totime";
		if($page > 1 && $sum) {
			$items = $sum;
		} else {
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}weixin_chat c,{$AJ_PRE}weixin_user u WHERE $condition");
			$items = $r['num'];
		}
		$pages = pages($items, $page, $pagesize);
		$lists = array();
		$result = $db->query("SELECT u.username,u.nickname,u.sex,u.city,u.province,u.country,u.language,u.headimgurl,c.* FROM {$AJ_PRE}weixin_chat c,{$AJ_PRE}weixin_user u WHERE $condition ORDER BY c.itemid DESC LIMIT $offset,$pagesize");
		while($r = $db->fetch_array($result)) {
