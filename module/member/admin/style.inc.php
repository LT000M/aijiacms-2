...
switch($action) {
	...
	case 'show':
		$itemid or msg();
		$u = $db->get_one("SELECT c.username FROM {$AJ_PRE}company c,{$AJ_PRE}member m WHERE c.userid=m.userid AND c.vip>0 AND m.edittime>0 ORDER BY m.logintimes DESC");
		if($u) dheader(AJ_PATH.'index.php?homepage='.$u['username'].'&preview='.$itemid);
		msg('暂无符合条件的会员');
	break;
	case 'order':
		$do->order($listorder);
	...
	default:
		...
		$sorder  = array('结果排序方式', '模板价格降序', '模板价格升序', $AJ['money_name'].'收益降序', $AJ['money_name'].'收益升序', $AJ['credit_name'].'收益降序', $AJ['credit_name'].'收益升序', '使用人数降序', '使用人数升序', '添加时间降序', '添加时间升序');
