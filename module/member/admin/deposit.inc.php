	...
	case 'add':
		if($submit) {
			...
			foreach($usernames as $username) {
				...
				$r = $db->get_one("SELECT username,deposit FROM {$AJ_PRE}member WHERE username='$username'");
				if(!$r) {
				...
				$db->query("INSERT INTO {$AJ_PRE}finance_deposit (username,amount,addtime,editor,reason,note) VALUES ('$username','$amount','$AJ_TIME','$_username','$reason','$note')");
				$db->query("UPDATE {$AJ_PRE}member SET deposit=deposit+$amount WHERE username='$username'");
				$success++;
			}
