...
$menus = array (
    array($AJ['credit_name'].'奖惩', '?moduleid='.$moduleid.'&file='.$file.'&username='.$username.'&action=add'),
    array($AJ['credit_name'].'流水', '?moduleid='.$moduleid.'&file='.$file.'&username='.$username),
		...
		if($submit) {
			...
			foreach($usernames as $username) {
				...
				if(!$type && $r['credit'] < abs($amount)) {
					$error .= '<br/>会员['.$username.']'.$AJ['credit_name'].'不足，当前'.$AJ['credit_name'].'为:'.$r['credit'];
					continue;
				}
				$reason or $reason = '奖励';
				$note or $note = '手工';
