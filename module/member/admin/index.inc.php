...
if(in_array($action, array('', 'check'))) {
	$sfields = array('按条件', '公司名', '会员名', '昵称','姓名', '手机号码', '部门', '职位', 'Email', 'QQ', '微信', '阿里旺旺', 'Skype', '注册IP', '登录IP', '客服专员', $AJ['trade_nm'], '推荐人', '备注');
	...
	$sorder  = array('结果排序方式', '注册时间降序', '注册时间升序', '修改时间降序', '修改时间升序', '登录时间降序', '登录时间升序', '登录次数降序', '登录次数升序', '账户'.$AJ['money_name'].'降序', '账户'.$AJ['money_name'].'升序', '会员'.$AJ['credit_name'].'降序', '会员'.$AJ['credit_name'].'升序', '短信余额降序', '短信余额升序');
	...
	$strade = array($AJ['trade_nm'], '已认证' , '未认证');
	...
	$vtrade_select = $AJ['trade_nm'] ? dselect($strade, 'vtrade', '', $vtrade) : '';
...
switch($action) {
	...
	case 'login':
		if($userid) {
			...
			$auth = encrypt($userid.'|'.$_username, AJ_KEY.'ADMIN');
			set_cookie('admin_user', $auth);
			msg('授权成功，正在转入会员会员中心...', $MODULE[2]['linkurl'].'?reload='.$AJ_TIME);
		} else {
			msg();
		}
	break;
	...
	case 'note_add':
		...
		$member or msg('会员不存在');
		if($member['note']) {
			$note = timetodate($AJ_TIME, 5)."|".$_username."|".$note."\n--------------------\n".addslashes($member['note']);
		} else {
			$note = timetodate($AJ_TIME, 5)."|".$_username."|".$note;
		}
