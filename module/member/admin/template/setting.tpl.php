...
<input type="radio" name="setting[checkuser]" value="4" <?php if($checkuser==4) echo 'checked';?>> 邮件或短信验证 &nbsp; <?php tips('社交账号和短信登录自动注册的账号不受此限制');?>
</td>
</tr>
...
<tr>
<td class="tl">异地登录短信安全验证</td>
<td>
<input type="radio" name="setting[verfiy_login]" value="1" <?php if($verfiy_login) echo 'checked';?>/> 开启&nbsp;&nbsp;
<input type="radio" name="setting[verfiy_login]" value="0" <?php if(!$verfiy_login) echo 'checked';?>/> 关闭 <?php tips('会员异地账号密码登录时，强制切换短信验证，社交账号登录不受此限制<br/>开启此设置请确保手机验证码登录已经开启，且只对已验证手机号的会员有效');?>
</td>
</tr>
<tr>
<td class="tl">用户登录有效期</td>
<td><input type="text" size="6" name="setting[login_time]" value="<?php echo $login_time;?>"/> 秒 <?php tips('0代表7天，大于0的数字代表记忆登录的时间，如果设置，最小值为86400（24小时）');?></td>
</tr>
<tr>
...
<input type="radio" name="setting[login_sms]" value="2" <?php if($login_sms == 2) echo 'checked';?>/> 开启并自动注册 <?php tips('开启并自动注册，代表手机号码未注册时自动创建会员账号');?>
</td>
</tr>
<tr>
...
<option value="Aijiacms"<?php if($editor == 'Aijiacms') echo ' selected';?>>精简</option>
...
<td><input type="text" name="setting[pay_banks]" style="width:95%;" value="<?php echo $pay_banks;?>"/><?php tips('手动添加'.$AJ['money_name'].'流水时需选择');?></td>
</tr>
<tr>
...
<table cellspacing="0" class="tb">
<tr>
<td class="tl"><?php echo $AJ['credit_name'];?>为负禁发信息</td>
<td>
<input type="radio" name="setting[credit_less]" value="0" <?php if(!$credit_less) echo 'checked';?>/> 关闭&nbsp;&nbsp;
<input type="radio" name="setting[credit_less]" value="1" <?php if($credit_less) echo 'checked';?>/> 开启<?php tips($AJ['credit_name'].'为负时禁止新发布信息，此项对游客无效');?>
</td>
</tr>
<tr>
...
<tr>
<td class="tl">24小时引导<?php echo $AJ['credit_name'];?>上限</td>
<td>
<input type="text" size="5" name="setting[credit_maxip]" value="<?php echo $credit_maxip;?>"/>
<?php tips('为了防止作弊，超过'.$AJ['credit_name'].'上限将不再计算');?>
</td>
</tr>
<tr>
<td class="tl">在线充值1<?php echo $AJ['money_unit'];?>奖励</td>
<td>
<input type="text" size="5" name="setting[credit_charge]" value="<?php echo $credit_charge;?>"/> <?php tips('每充值1'.$AJ['money_unit'].' 奖励对应倍数的'.$AJ['credit_name']);?>
</td>
</tr>
<tr>
...
<div class="tt"><?php echo $AJ['credit_name'];?>购买</div>
<table cellspacing="0" class="tb">
<tr>
<td class="tl"><?php echo $AJ['credit_name'];?>购买额度</td>
<td>
<input type="text" size="50" name="setting[credit_buy]" value="<?php echo $credit_buy;?>"/>
</td>
</tr>
<tr>
<td class="tl"><?php echo $AJ['credit_name'];?>对应价格</td>
<td>
<input type="text" size="50" name="setting[credit_price]" value="<?php echo $credit_price;?>"/><br/>
<span class="f_gray"><?php echo $AJ['credit_name'];?>和价格用|分隔，二者必须一一对应</span>
</td>
</tr>
</table>
<div class="tt"><?php echo $AJ['credit_name'];?>兑换</div>
<table cellspacing="0" class="tb">
<tr>
...
<tr>
<td class="tl">兑换比率</td>
<td><input name="setting[ex_rate]" type="text" size="15" value="<?php echo $ex_rate;?>"/><?php tips('例如填5表示1个论坛积分可兑换5个'.$AJ['credit_name']);?></td>
</tr>
<tr>
...
<td><input name="setting[passport_url]" type="text" size="50" value="<?php echo $passport_url;?>"/><?php tips('整合程序接口地址 例如:http://bbs.aijiacms.com 结尾不要带斜线');?></td>
</tr>
<tr>
<td class="tl">整合密钥</td>
...
<td><input name="setting[uc_api]" type="text" size="50" value="<?php echo $uc_api;?>" id="uc_api"/><?php tips('整合程序接口地址 例如:http://bbs.aijiacms.com 结尾不要带斜线');?></td>
</tr>
<tr>
<td class="tl">主机IP</td>
