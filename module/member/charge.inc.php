...
function get_reason_url($r = array()) {
	...
	switch($arr[0]) {
		case 'deposit':
			$url = 'deposit.php?action=add&auth='.encrypt($r['reason'], AJ_KEY.'CG', 600);
		break;
		case 'credit':
			$url = 'credit.php?action=buy&auth='.encrypt($r['reason'], AJ_KEY.'CG', 600);
		break;
		case 'sms':
			$url = 'sms.php?action=buy&auth='.encrypt($r['reason'], AJ_KEY.'CG', 600);
		break;
		case 'vip':
			$url = 'account.php?action=vip&auth='.encrypt($r['reason'], AJ_KEY.'CG', 600);
		break;
		case 'grade':
			//$url = 'account.php?action=grade&groupid='.intval($arr[1]).'&auth='.encrypt($r['reason'], AJ_KEY.'CG', 600);
        $url = 'account.php?action=grade&groupid='.intval($arr[1]).'&buy_month='.$r['amount'].'&auth='.encrypt($r['reason'], AJ_KEY.'CG', 600);
		break;
		case 'style':
			if(is_numeric($arr[1]) && is_numeric($arr[2])) $url = 'style.php?action=buy&itemid='.intval($arr[1]).'&auth='.encrypt($r['reason'], AJ_KEY.'CG', 600);
		break;
		case 'spread':
			if(is_numeric($arr[1]) && $arr[2]) $url = 'spread.php?action=add&mid='.intval($arr[1]).'&word='.urlencode(decrypt($arr[2], AJ_KEY.'CR')).'&auth='.encrypt($r['reason'], AJ_KEY.'CG', 600);
		break;
		case 'pay':
			if(is_numeric($arr[1]) && is_numeric($arr[2])) $url = 'pay.php?mid='.intval($arr[1]).'&itemid='.intval($arr[2]).'&auth='.encrypt($r['reason'], AJ_KEY.'CG', 600);
		break;
		case 'award':
			if(is_numeric($arr[1]) && is_numeric($arr[2])) $url = 'award.php?mid='.intval($arr[1]).'&itemid='.intval($arr[2]).'&auth='.encrypt($r['reason'], AJ_KEY.'CG', 600);
		break;
		case 'trade':
			if(is_numeric($arr[1])) $url = 'order.php?action=update&step=pay&itemid='.intval($arr[1]).'&auth='.encrypt($r['reason'], AJ_KEY.'CG', 600);
		break;
		case 'trades':
			$url = 'order.php?action=muti&step=pay&auth='.encrypt($r['reason'], AJ_KEY.'CG', 600);
		break;
		case 'group':
			if(is_numeric($arr[1])) $url = 'deal.php?mid='.intval($arr[2]).'&action=update&step=pay&itemid='.intval($arr[1]).'&auth='.encrypt($r['reason'], AJ_KEY.'CG', 600);
		break;
		default:
		break;
	}
...
function get_reason($reason) {
	...
	switch($arr[0]) {
		...
		case 'trade':
			if(is_numeric($arr[1])) {
				$t = DB::get_one("SELECT title FROM ".AJ_PRE."order WHERE itemid=".intval($arr[1]));
				if($t) $str = $t['title'];
			}
		break;
		case 'trades':
			$ids = explode(',', $arr[1]);
			$t = DB::get_one("SELECT title FROM ".AJ_PRE."order WHERE itemid=".intval($ids[0]));
			if($t) $str = $L['charge_reason_muti'].$t['title'].'...';
		break;
		case 'group':
			if(is_numeric($arr[1])) {
				$t = DB::get_one("SELECT title FROM ".AJ_PRE."group_order_".intval($arr[2])." WHERE itemid=".intval($arr[1]));
				if($t) $str = $t['title'];
			}
		break;
		default:
...
switch($action) {
	case 'record':
		...
		while($r = $db->fetch_array($result)) {
			$r['repay'] = ($r['status'] == 0 && $AJ_TIME - $r['sendtime'] > 600) ? 1 : 0;
			if($r['repay']) $repay = 1;
	...
	case 'repay':
		...
		($r && $r['status'] == 0 && $AJ_TIME - $r['sendtime'] > 600 && $_username == $r['username']) or dheader('?action=record');
		$amount = $r['amount'];
		$fee = $r['fee'];
		$charge = $fee + $amount;
		$bank = $r['bank'];
	...
	case 'confirm':
		...
		if($auto) $goto = 1;
		if(isset($goto)) {
			$receive_url = ($AJ_PC ? $MOD['linkurl'] : $MOD['mobile']).'charge.php';
			$charge_title = get_reason($reason);
			$db->query("INSERT INTO {$AJ_PRE}finance_charge (username,bank,amount,fee,sendtime,reason) VALUES ('$_username','$bank','$amount','$fee','$AJ_TIME','$reason')");
	...
	default:
		...
		*/
		...
		if($pay_id) {
			$r = $db->get_one("SELECT * FROM {$AJ_PRE}finance_charge WHERE itemid=$pay_id");
