...
switch($action) {
	...
	case 'edit':
		...
		if($submit) {
			...
			$sql = substr($sql, 1);
			$db->query("UPDATE {$AJ_PRE}ask SET $sql WHERE itemid=$itemid");
			clear_upload($content, $itemid, 'ask');
	...
	case 'star':
		...
		$db->query("UPDATE {$AJ_PRE}ask SET star=$star WHERE itemid=$itemid");
	...
	default:
		...
		$lists = array();
		$result = $db->query("SELECT * FROM {$AJ_PRE}ask WHERE $condition ORDER BY itemid DESC LIMIT $offset,$pagesize");
