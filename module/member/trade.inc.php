...
$memberurl = $AJ_PC ? $MOD['linkurl'] : $MOD['mobile'];
...
if($action == 'update') {
	...
	$td['linkurl'] = ($AJ_PC ? AJ_PATH : AJ_MOB).'api/redirect.php?mid='.$td['mid'].'&itemid='.$td['mallid'];
	$td['par'] = '';
	...
	if(($td['amount'] + $td['discount']) > $td['price']*$td['number']) {
		...
		while($r = $db->fetch_array($result)) {
			$r['linkurl'] = ($AJ_PC ? AJ_PATH : AJ_MOB).'api/redirect.php?mid='.$r['mid'].'&itemid='.$r['mallid'];
			$r['par'] = '';
	...
	switch($step) {
		case 'edit_price'://修改价格||确认订单||修改为货到付款
			...
			if($submit) {
				...
				$db->query("UPDATE {$table} SET fee='$fee',fee_name='$fee_name',status=$status,cod=$cod,updatetime=$AJ_TIME WHERE itemid=$itemid");
				foreach($lists as $k=>$v) {
					if($v['itemid'] != $itemid) $db->query("UPDATE {$table} SET status=$status,cod=$cod,updatetime=$AJ_TIME WHERE itemid=$v[itemid]");
				}
		...
		break;
		case 'detail'://订单详情
			$auth = encrypt('mall|'.$td['send_type'].'|'.$td['send_no'].'|'.$td['send_status'].'|'.$td['itemid'], AJ_KEY.'EXPRESS');
		...
		case 'refund_agree'://卖家同意买家退款
			...
			if($submit) {
				...
				foreach($lists as $k=>$v) {
					$db->query("UPDATE {$table} SET status=6,editor='$_username',updatetime=$AJ_TIME,refund_reason='$content' WHERE itemid=$v[itemid]");
				}
				//更新商品数据 增加库存
				foreach($lists as $k=>$v) {
		...
		case 'send_goods'://卖家发货
			...
			if($submit) {
				...
				foreach($lists as $k=>$v) {
					$db->query("UPDATE {$table} SET status=$status,updatetime=$AJ_TIME,send_type='$send_type',send_no='$send_no',send_time='$send_time' WHERE itemid=$v[itemid]");
				}
				$touser = $td['buyer'];
		...
		case 'cod_success'://货到付款，确认完成
			...
			foreach($lists as $k=>$v) {
				$db->query("UPDATE {$table} SET status=4,updatetime=$AJ_TIME WHERE itemid=$v[itemid]");
			}
			//交易成功
		...
		case 'get_pay'://买家确认超时 卖家申请直接付款
			...
			//网站服务费
			$G = $db->get_one("SELECT groupid FROM {$AJ_PRE}member WHERE username='".$td['seller']."'");
		...
		case 'comment'://交易评价
			...
			if($submit) {
				foreach($lists as $k=>$v) {
					...
					$s = 'b'.$star;
					$db->query("UPDATE {$AJ_PRE}mall_comment_".$mid." SET buyer_star=$star,buyer_comment='$content',buyer_ctime=$AJ_TIME WHERE itemid=$itemid");
					$db->query("UPDATE {$AJ_PRE}mall_stat_".$mid." SET bcomment=bcomment+1,`$s`=`$s`+1 WHERE mallid=$mallid");
				}
		...
		case 'comment_detail'://评价详情
			...
			foreach($lists as $k=>$v) {
				$id = $v['itemid'];
				$c = $db->get_one("SELECT * FROM {$AJ_PRE}mall_comment_$v[mid] WHERE itemid=$id");
				$comments[$k] = $c;
				$O[$id] = $v;
				$C[$id] = $c;
			}
			if($submit) {
				...
				if(isset($C[$oid])) {
					...
					$db->query("UPDATE {$AJ_PRE}mall_comment_".$O[$oid]['mid']." SET buyer_reply='$content',buyer_rtime=$AJ_TIME WHERE itemid=$oid");
		...
		case 'close'://关闭交易
			if($td['status'] == 0) {
				foreach($lists as $k=>$v) {
					$db->query("UPDATE {$table} SET status=9,updatetime=$AJ_TIME WHERE itemid=$v[itemid]");
				}
	...
	if($pids) {
		...
		while($r = $db->fetch_array($result)) {
			...
			$r['par'] = '';
			if(strpos($r['note'], '|') !== false) list($r['note'], $r['par']) = explode('|', $r['note']);
			$r['addtime'] = timetodate($r['addtime'], $AJ_PC ? 5 : 3);
			$r['linkurl'] = ($AJ_PC ? AJ_PATH : AJ_MOB).'api/redirect.php?mid='.$r['mid'].'&itemid='.$r['mallid'];
			$r['dstatus'] = $_status[$r['status']];
			$r['money'] = $r['amount'] + $r['fee'];
			$r['money'] = number_format($r['money'], 2, '.', '');
