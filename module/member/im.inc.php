...
switch($action) {
	...
	case 'setting':
		if($submit) {
			...
			$db->query("UPDATE {$AJ_PRE}member_misc SET black='$black',reply='$reply' WHERE userid=$_userid");
			userclean($_username);
			dmsg($L['op_update_success'], '?action=setting');
		} else {
			$user = $db->get_one("SELECT black,reply FROM {$AJ_PRE}member_misc WHERE userid=$_userid");
		}
	break;
	case 'view':
	...
	case 'list':
		...
		if($new != $_chat) {
			$db->query("UPDATE {$AJ_PRE}member SET chat=$new WHERE userid=$_userid");
			$_chat = $new;
		}
