...
if($action == 'update') {
	...
	$td['total'] = $td['amount'];
	$td['linkurl'] = ($AJ_PC ? AJ_PATH : AJ_MOB).'api/redirect.php?mid='.$mid.'&itemid='.$td['gid'];
	$td['fee_name'] = $td['fee'] = $td['par'] = '';
	$lists = array($td);
	$gid = $td['gid'];
	switch($step) {
		case 'detail':
			$auth = encrypt('group|'.$td['send_type'].'|'.$td['send_no'].'|'.$td['send_status'].'|'.$td['itemid'], AJ_KEY.'EXPRESS');
		...
		case 'used':
			if($td['seller'] == $_username) {
				...
				$date = timetodate($AJ_TIME, 6);
				$db->query("UPDATE {$table} SET status=2,send_time='$date',updatetime=$AJ_TIME WHERE itemid=$itemid");
		...
		case 'pay'://买家付款
			...
			if($submit) {
				...
				$db->query("UPDATE {$table} SET status=0,password='$password',updatetime=$AJ_TIME WHERE itemid=$itemid");
				if($password) {
					//send sms
					if($AJ['sms']) {
		...
		case 'refund'://买家退款
			...
			if($submit) {
				...
				is_payword($_username, $password) or message($L['error_payword']);
				$db->query("UPDATE {$table} SET status=4,updatetime=$AJ_TIME,buyer_reason='$content' WHERE itemid=$itemid");
	...
	while($r = $db->fetch_array($result)) {
		...
		$r['addtime'] = str_replace(' ', '<br/>', timetodate($r['addtime'], $AJ_PC ? 5 : 3));
		$r['updatetime'] = str_replace(' ', '<br/>', timetodate($r['updatetime'], 5));
		$r['linkurl'] = ($AJ_PC ? AJ_PATH : AJ_MOB).'api/redirect.php?mid='.$mid.'&itemid='.$r['gid'];
