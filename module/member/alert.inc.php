...
require AJ_ROOT.'/module/'.$module.'/alert.class.php';
$do = new alert();

switch($action) {
	case 'add':
		if($MG['alert_limit']) {
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}alert WHERE username='$_username' AND status>0");
	...
	default:
		...
		if($lists) {
			foreach($lists as $k=>$v) {
				if($v['catid']) $lists[$k]['cate'] = $AJ_PC ? cat_pos(get_cat($v['catid']), '-', 1) : strip_tags(cat_pos(get_cat($v['catid']), '-'));
				if($v['email'] != $_email) $db->query("UPDATE {$AJ_PRE}alert SET email='$_email' WHERE itemid=$v[itemid]");
			}
		}
...
$limit_used = 0;
for($i = 2; $i < 4; $i++) {
	$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}alert WHERE username='$_username' AND status=$i");
	$nums[$i] = $r['num'];
	$limit_used += $r['num'];
}
