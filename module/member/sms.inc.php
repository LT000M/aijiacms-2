...
login();
$AJ['sms'] or message($L['feature_close']);
...
$_user = $db->get_one("SELECT mobile,vmobile FROM {$AJ_PRE}member WHERE userid=$_userid");
...
switch($action) {
		...
		if($fee) {
			...
			if($submit) {
				...
				if($amount > 0) {
					...
					sms_record($_username, $total, 'system', $L['sms_buy_record'], $amount.$AJ['money_unit']);
				}
	...
	case 'record':
		...
		while($r = $db->fetch_array($result)) {
			...
			$r['sendtime'] = $AJ_PC ? str_replace(' ', '<br/>', timetodate($r['sendtime'], 6)) : timetodate($r['sendtime'], 6);
	...
	default:
		...
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}finance_sms WHERE $condition");
		$items = $r['num'];
		$pages = pages($items, $page, $pagesize);		
		$lists = array();
		$result = $db->query("SELECT * FROM {$AJ_PRE}finance_sms WHERE $condition ORDER BY itemid DESC LIMIT $offset,$pagesize");
		$income = $expense = 0;
		while($r = $db->fetch_array($result)) {
