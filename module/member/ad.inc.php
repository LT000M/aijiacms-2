...
if($action == 'add') {
	...
	if($pid) {
		...
		$p = $db->get_one("SELECT * FROM {$AJ_PRE}ad_place WHERE pid=$pid");
		$p or message($L['not_ad_place'], $adurl);
		$typeid = $p['typeid'];
		$price = $p['price'];
		$months = array(1, 2, 3, 6, 12, 24);
		$t = $db->get_one("SELECT MAX(totime) AS totime FROM {$AJ_PRE}ad WHERE pid=$pid AND totime>$AJ_TIME");
		$fromtime = ($t['totime'] && $typeid < 5) ? $t['totime'] : $AJ_TIME + 86400;
		...
		if($submit) {
			...
			$ad['addtime'] = $ad['edittime'] = $AJ_TIME;
			...
			$ad['introduce'] = timetodate($AJ_TIME, 5).' '.$L['ad_buy_paid'].$amount.$unit;
			$ad['note'] = $post['note'].' - '.$L['ad_buy_note'].'('.$AJ_IP.')';
			$ad['status'] = 2;
			...
			$sqlk = substr($sqlk, 1);
			$sqlv = substr($sqlv, 1);
			$db->query("INSERT INTO {$AJ_PRE}ad ($sqlk) VALUES ($sqlv)");
	...
	$unit = $currency == 'money' ? $AJ['money_unit'] : $AJ['credit_unit'];
	$condition = "open=1";
	$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}ad_place WHERE $condition");
	$items = $r['num'];
	$pages = pages($items, $page, $pagesize);
	$lists = array();
	$result = $db->query("SELECT * FROM {$AJ_PRE}ad_place WHERE $condition ORDER BY listorder DESC,pid DESC LIMIT $offset,$pagesize");
	...
	$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}ad WHERE $condition");
	$items = $r['num'];
	$pages = pages($items, $page, $pagesize);
	$lists = array();
	$result = $db->query("SELECT * FROM {$AJ_PRE}ad WHERE $condition ORDER BY aid DESC LIMIT $offset,$pagesize");
	while($r = $db->fetch_array($result)) {
		if($r['totime'] < $AJ_TIME) {
...
$nums = array();
for($i = 2; $i < 4; $i++) {
	$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}ad WHERE username='$_username' AND status=$i");
	$nums[$i] = $r['num'];
}
if($AJ_PC) {
	//
