...
require AJ_ROOT.'/module/'.$module.'/style.class.php';
$do = new style();
...
switch($action) {
	case 'buy':
		...
		if($submit) {			
			...
			$styletime = $AJ_TIME + 86400*30*$month;
			$o = $db->get_one("SELECT itemid FROM {$AJ_PRE}style WHERE skin='$skin' AND template='$template'");
			if($o) $db->query("UPDATE {$AJ_PRE}style SET hits=hits-1 WHERE itemid=$o[itemid] AND hits>1");			
			$db->query("UPDATE {$AJ_PRE}style SET hits=hits+1,`$fd`=`$fd`+$amount WHERE itemid=$itemid");
			$db->query("UPDATE {$AJ_PRE}company SET template='$r[template]',skin='$r[skin]',styletime=$styletime WHERE userid=$_userid");
			userclean($_username);
			dmsg($L['style_msg_buy_success'], '?action=index');
		} else {
			$r['thumb'] = is_file(AJ_ROOT.'/'.$MODULE[4]['moduledir'].'/skin/'.$r['skin'].'/thumb.gif') ? $MODULE[4]['linkurl'].'skin/'.$r['skin'].'/thumb.gif' : $MODULE[4]['linkurl'].'image/nothumb.gif';
			extract($r);
	...
	case 'choose':
		...
		$o = $db->get_one("SELECT itemid FROM {$AJ_PRE}style WHERE skin='$skin' AND template='$template'");
		if($o) $db->query("UPDATE {$AJ_PRE}style SET hits=hits-1 WHERE itemid=$o[itemid] AND hits>1");
		$db->query("UPDATE {$AJ_PRE}style SET hits=hits+1 WHERE itemid=$itemid");
		$db->query("UPDATE {$AJ_PRE}company SET template='$r[template]',skin='$r[skin]',styletime=0 WHERE userid=$_userid");
		userclean($_username);
		dmsg($L['style_msg_use_success'], $forward);
	break;		
	case 'view':
		$c = $db->get_one("SELECT * FROM {$AJ_PRE}style WHERE skin='$skin' AND template='$template'");
		$c['thumb'] = is_file(AJ_ROOT.'/'.$MODULE[4]['moduledir'].'/skin/'.$c['skin'].'/thumb.gif') ? $MODULE[4]['linkurl'].'skin/'.$c['skin'].'/thumb.gif' : $MODULE[4]['linkurl'].'image/nothumb.gif';
		$havedays = $user['styletime'] ? ceil(($user['styletime']-$AJ_TIME)/86400) : 0;
	break;
	default:
		...
		if(!$skin) {
			if($MG['styleid']) {
				$o = $db->get_one("SELECT skin FROM {$AJ_PRE}style WHERE itemid='$MG[styleid]'");
				if($o) $skin = $o['skin'];
			}
		}
