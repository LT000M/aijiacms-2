...
function update_company_setting($userid, $setting) {
	...
	foreach($setting as $k=>$v) {
		...
		if(isset($S[$k])) {
			DB::query("UPDATE ".AJ_PRE."company_setting SET item_value='$v' WHERE userid=$userid AND item_key='$k'");
		} else {
			DB::query("INSERT INTO ".AJ_PRE."company_setting (userid,item_key,item_value) VALUES ('$userid','$k','$v')");
		}
	}
	return true;
}
...
function get_company_setting($userid, $key = '', $cache = '') {
	if($key) {
		$r = DB::get_one("SELECT * FROM ".AJ_PRE."company_setting WHERE userid=$userid AND item_key='$key'", $cache);
		return $r ? $r['item_value'] : '';
	} else {
		$setting = array();
		$query = DB::query("SELECT * FROM ".AJ_PRE."company_setting WHERE userid=$userid", $cache);
...
function get_paylist() {
	...
	if($AJ_MOB['browser'] == 'weixin' && $PAY['weixin']['enable']) {
		$tmp = $PAY['weixin'];
		unset($PAY['weixin']);
