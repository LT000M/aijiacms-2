...
if($action == 'add') {
	...
	if($word && $mid > 3 && isset($MODULE[$mid]) && !$MODULE[$mid]['islink']) {
		...
		$r = $db->get_one("SELECT * FROM {$AJ_PRE}spread WHERE username='$_username' AND mid=$mid AND word='$word' AND fromtime>=$next_time");
		if($r) message($L['spread_msg_buy'], $AJ_PC ? $EXT['spread_url'] : $EXT['spread_mob']);
		$mid or $mid = 5;
		$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}spread WHERE mid=$mid AND status=3 AND word='$word' AND fromtime>=$next_time");
		if($r['num'] > $spread_max) message(lang($L['spread_msg_over'], array($word)), $AJ_PC ? $EXT['spread_url'] : $EXT['spread_mob']);

		$price = dround($EXT['spread_price']);
		$p1 = $db->get_one("SELECT * FROM {$AJ_PRE}spread_price WHERE mid=$mid AND word='$word' ORDER BY edittime DESC");
		if($p1) {
			$price = $p1['price'];
		} else {
			$p2 = $db->get_one("SELECT * FROM {$AJ_PRE}spread_price WHERE mid=$mid AND word='' ORDER BY edittime DESC");
		...
		if($submit) {
			...
			$db->query("INSERT INTO {$AJ_PRE}spread (mid,tid,word,price,currency,company,username,addtime,fromtime,totime,status) VALUES ('$mid','$buy_tid','$word','$buy_price','$currency','$_company','$_username','$AJ_TIME','$next_time','$totime','$status')");
	...
	$lists = array();
	$result = $db->query("SELECT * FROM {$AJ_PRE}spread WHERE $condition ORDER BY itemid DESC LIMIT $offset,$pagesize");
	while($r = $db->fetch_array($result)) {
		if($r['totime'] < $AJ_TIME) {
			...
			$r['days'] = $r['totime'] > $AJ_TIME ? intval(($r['totime']-$AJ_TIME)/86400) : 0;
		$lists[] = $r;
	}
}
...
$nums = array();
for($i = 2; $i < 4; $i++) {
	$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}spread WHERE username='$_username' AND status=$i");
	$nums[$i] = $r['num'];
}
if($AJ_PC) {
	//
