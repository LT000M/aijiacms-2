...
require AJ_ROOT.'/module/'.$module.'/page.class.php';
$do = new page();
switch($action) {
	case 'add':
		...
		if($MG['hour_limit']) {
			$today = $AJ_TIME - 3600;
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}page WHERE username='$_username' AND addtime>$today");
		...
		if($MG['page_limit']) {
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}page WHERE username='$_username' AND status>0");
...
$limit_used = 0;
for($i = 1; $i < 4; $i++) {
	$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}page WHERE username='$_username' AND status=$i");
	$nums[$i] = $r['num'];
	$limit_used += $r['num'];
}
