...
class style {
	...
	function pass($post) {
		...
		if(!is_file(AJ_ROOT.'/'.$MODULE[4]['moduledir'].'/skin/'.$post['skin'].'/style.css')) return $this->_($L['style_pass_css']);
		...
		if(!is_file(AJ_ROOT.'/template/'.$CFG['template'].'/'.$post['template'].'/side_search.htm')) return $this->_($L['style_pass_dir']);
	...
	function delete($itemid, $all = true) {
		if(is_array($itemid)) {
			...
			DB::query("UPDATE ".AJ_PRE."company SET skin='',template='' WHERE skin='".$r['skin']."' AND template='".$r['template']."'");
