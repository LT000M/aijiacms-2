...
require AJ_ROOT.'/module/'.$module.'/favorite.class.php';
$do = new favorite();
switch($action) {
	case 'add':
		if($MG['favorite_limit']) {
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}favorite WHERE userid=$_userid");
		...
		if($mid > 4 && $itemid) {
			...
			if($t && $t['status'] > 2) {
				...
				if($f) {
					...
					$db->query("UPDATE {$AJ_PRE}favorite SET title='$title',url='$url',thumb='$thumb',addtime='$AJ_TIME' WHERE itemid=$f[itemid]");
