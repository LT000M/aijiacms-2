...
class message {
	...
	function is_message($message) {
		...
		if(AJ_MAX_LEN && strlen(clear_img($message['content'])) > AJ_MAX_LEN) return $this->_(lang('message->pass_max'));
		return true;
	}
	...
	function send($message) {
		...
		if(isset($message['save'])) {
			DB::query("INSERT INTO ".AJ_PRE."message(title,typeid,content,fromuser,touser,addtime,ip,status) values('$message[title]','$message[typeid]','$message[content]','$this->username','$message[touser]','".AJ_TIME."','".AJ_IP."','1')");
		} else {
			...
			foreach(explode(' ', $message['touser']) as $touser) {
				...
				$user = DB::get_one("SELECT black FROM ".AJ_PRE."member_misc WHERE username='$touser'");
				if($user) {
					$blacks = $user['black'] ? explode(' ', $user['black']) : array();
					if(!in_array($this->username, $blacks) && !in_array($touser, $tousers)) {
						$tousers[] = $touser;
						if(isset($message['copy'])) DB::query("INSERT INTO ".AJ_PRE."message (title,typeid,content,fromuser,touser,addtime,ip,feedback,status) VALUES ('$message[title]','$message[typeid]','$message[content]','$this->username','$touser','".AJ_TIME."','".AJ_IP."','$feedback','2')");
						DB::query("UPDATE ".AJ_PRE."member SET message=message+1 WHERE username='$touser'");
						DB::query("INSERT INTO ".AJ_PRE."message (title,typeid,content,fromuser,touser,addtime,ip,feedback,status) VALUES ('$message[title]','$message[typeid]','$message[content]','$this->username','$touser','".AJ_TIME."','".AJ_IP."','$feedback','3')");
					}
				}
			}
		}
	...
	function edit($message) {
		...
		DB::query("UPDATE ".AJ_PRE."message SET title='$message[title]',content='$message[content]' WHERE itemid='$this->itemid' ");
		if(isset($message['send'])) return $this->send($message);
		return true;
	}

	function get_one() {
        return DB::get_one("SELECT * FROM ".AJ_PRE."message WHERE itemid='$this->itemid'");
	}
	...
	function get_list($condition, $order = 'itemid DESC') {
		...
		$messages = array();
		$result = DB::query("SELECT * FROM ".AJ_PRE."message WHERE $condition ORDER BY $order LIMIT $offset,$pagesize");
	...
	function get_sys() {
		global $_groupid, $L;
		$messages = array();
		$result = DB::query("SELECT * FROM ".AJ_PRE."message WHERE groupids<>'' ORDER BY itemid DESC", 'CACHE');
		while($r = DB::fetch_array($result)) {
	...
	function export($message) {
		...
		$data = '';
		$result = DB::query("SELECT * FROM ".AJ_PRE."message WHERE $condition ORDER BY itemid DESC Limit 100");
		while($r = DB::fetch_array($result)) {
			$r['adddate'] = timetodate($r['addtime'], $L['message_list_date']);
		...
		if($data) {
			...
			$filename = 'message-'.timetodate(AJ_TIME, 'YmdHis');
			$data = '<html><meta http-equiv="Content-Type" content="text/html;charset='.AJ_CHARSET.'"/><title>'.$this->username.' '.$names[$status].''.$AJ['sitename'].' '.timetodate(AJ_TIME, 5).' - Powered By AIJIACMS.COM</title><style type="text/css">*{font-size:13px;font-family:Verdana,Arial;}body{width:750px;margin:auto;line-height:200%;}</style><base target="_blank"/><base href="'.AJ_PATH.'"/><body><br/>'.$data.'<br/></body></html>';
			ob_start();
			header('Cache-control: max-age=31536000');
			header('Expires: '.gmdate('D, d M Y H:i:s', AJ_TIME + 31536000).' GMT');
	...
	function clear($status) {
		if($status == 4 || $status == 3) {
			DB::query("DELETE FROM ".AJ_PRE."message WHERE status='$status' AND touser='$this->username' ");
			if($status == 3) DB::query("UPDATE ".AJ_PRE."member SET message=0 WHERE username='$this->username' ");
		} else if($status == 2 || $status == 1) {			
			DB::query("DELETE FROM ".AJ_PRE."message WHERE status='$status' AND fromuser='$this->username' ");
		}
	}
	...
	function delete($recycle = 0) {
		...
		$result = DB::query("SELECT * FROM ".AJ_PRE."message WHERE itemid IN($itemids) ORDER BY itemid DESC");
		while($r = DB::fetch_array($result)) {
			if(defined('AJ_ADMIN')) {
				if($r['status'] == 3 && !$r['isread']) DB::query("UPDATE ".AJ_PRE."member SET message=message-1 WHERE username='$r[touser]' ");
				DB::query("DELETE FROM ".AJ_PRE."message WHERE itemid='$r[itemid]'");
			} else {
				if($r['status'] == 4) {
					...
					if($this->username == $r['touser']) {
						if($recycle) {
							DB::query("UPDATE ".AJ_PRE."message SET status=4 WHERE itemid='$r[itemid]' ");
						} else {
							$this->_delete($r['itemid']);
						}
						if(!$r['isread']) DB::query("UPDATE ".AJ_PRE."member SET message=message-1 WHERE username='$this->username' ");
					}
	...
	function mark() {
		...
		$r = DB::get_one("SELECT COUNT(*) AS num FROM ".AJ_PRE."message WHERE $condition");
		if($r['num']) {
			DB::query("UPDATE ".AJ_PRE."message SET isread=1 WHERE $condition");
			DB::query("UPDATE ".AJ_PRE."member SET message=message-$r[num] WHERE username='$this->username' ");
		}
	}

	function markall() {
		DB::query("UPDATE ".AJ_PRE."message SET isread=1 WHERE status=3 AND isread=0 AND touser='$this->username'");
		DB::query("UPDATE ".AJ_PRE."member SET message=0 WHERE username='$this->username' ");
	}
	...
	function restore() {
		...
		while($r = DB::fetch_array($result)) {
			if($r['status'] == 4 && $this->username == $r['touser']) {
				DB::query("UPDATE ".AJ_PRE."message SET status=3 WHERE itemid='$r[itemid]' ");				
				if(!$r['isread']) DB::query("UPDATE ".AJ_PRE."member SET message=message+1 WHERE username='$this->username' ");
			}
		}
	}
	...
	function read() {
		DB::query("UPDATE ".AJ_PRE."message SET isread=1 WHERE itemid='$this->itemid'");
		DB::query("UPDATE ".AJ_PRE."member SET message=message-1 WHERE userid='$this->userid'");
	}

	function color($style) {
		$message = $this->get_one();
		if($message['status'] == 3 && $message['touser'] == $this->username) {
			DB::query("UPDATE ".AJ_PRE."message SET style='$style' WHERE itemid='$this->itemid'");
		}
	}
	...
	function feedback($r) {
		...
		$message['content'] = lang($L['message_feedback_content'], array($this->username, timetodate(AJ_TIME, 5), $r['title'], timetodate($r['addtime'], 5), $r['content']));
		$this->send($message);
	}

	function fix_message() {
		global $_username, $_message;
		$r = DB::get_one("SELECT COUNT(*) AS num FROM ".AJ_PRE."message WHERE touser='$_username' AND status=3 AND isread=0");
		$num = intval($r['num']);
		if($_message != $num) {
			DB::query("UPDATE ".AJ_PRE."member SET message='$num' WHERE username='$_username'");
			dheader('message.php');
		}
	}
	...
	function _send($message) {
		...
		if($message['type']) {
			...
			DB::query("INSERT INTO ".AJ_PRE."message(title,content,fromuser,touser,addtime,status,groupids) values('$message[title]','$message[content]','$this->username','','".AJ_TIME."','0','$message[groupids]')");
		} else {
			foreach(explode(' ', $message['touser']) as $touser) {
	...
	function _edit($message) {
		...
		$message['content'] = dsafe(addslashes(save_remote(save_local(stripslashes($message['content'])))));
		$message['groupids'] = implode(',', $message['groupids']);
		DB::query("UPDATE ".AJ_PRE."message SET title='$message[title]',content='$message[content]',groupids='$message[groupids]' WHERE itemid='$this->itemid' ");
		return true;
	}
	...
	function _clear($message) {
		...
		DB::query("DELETE FROM ".AJ_PRE."message WHERE $condition");
		return true;
	}
	...
	function _delete($itemid) {
		...
		DB::query("DELETE FROM ".AJ_PRE."message WHERE itemid='$itemid' ");
	}
