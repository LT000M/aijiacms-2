...
function email_exists($email) {
	...
	function pass($member, $auto = 0) {
		...
		if($this->userid) {			
			...
			if(!$areaid || !DB::get_one("SELECT areaid FROM ".AJ_PRE."area WHERE areaid=$areaid")) return $this->_($L['member_areaid_null']);
		...
		if(AJ_MAX_LEN && strlen(clear_img($member['content'])) > AJ_MAX_LEN) return $this->_(lang('message->pass_max'));
		return true;
	}
	...
	function add($member) {
		...
		$member_fields = array('username','company','passport', 'password','payword','email','sound','gender','truename','mobile','qq','wx','wxqr','ali','skype','department','career','groupid','regid','areaid','edittime','inviter','passsalt', 'paysalt','companyid','oppid','weixin_openid','nickname','avatar','baidu_openid');
		...
		$pc = is_mob() ? '手机' : '电脑';
		keyuanfrom($member['truename'], $member['mobile'],$this->username,$this->userid,$pc);
		...
		$this->bind($member['username'], AJ_TIME);
	...
	function edit($member)	{
		$member = $this->set($member);
		$r = $this->get_one();
		$member_fields = array('company','passport','sound','email','qq','wx','wxqr','ali','skype','gender','truename','mobile','department','career','groupid','areaid', 'edittime','vemail','vmobile','vbank','vtruename','vcompany','vtrade','trade','support','inviter','companyid','oppid');
		$misc_fields = array('username','bank','banktype','branch','account','reply','black', 'send');
		$company_fields = array('company','type','areaid', 'catid','catids','business','mode','regyear','regunit','capital','size','address','postcode','telephone','fax','mail','gzh','gzhqr','homepage','sell','buy','introduce','thumb','keyword','linkurl','groupid','domain','icp','validated','validator','validtime','skin','template','letter');
	...
	function login($login_username, $login_password, $login_cookietime = 0, $reason = '', $child = 0) {
		...
		$auth = encrypt($user['userid'].'|'.$user['password'].'|'.$child, AJ_KEY.'USER');
		set_cookie('auth', $auth, $cookietime);
		set_cookie('username', $user['username'], AJ_TIME + 30*86400);
		DB::query("UPDATE {$this->table_member} SET loginip='".AJ_IP."',logintime=".AJ_TIME.",logintimes=logintimes+1 WHERE userid=$userid");
		if($child) DB::query("UPDATE {$this->table_member}_child SET loginip='".AJ_IP."',logintime=".AJ_TIME.",logintimes=logintimes+1 WHERE itemid=$child");
		$this->cart($userid);
		$this->bind($user['username'], AJ_TIME);
		return $user;
	}

	function cart($userid) {//SYNC
		$r = DB::get_one("SELECT data FROM ".AJ_PRE."cart WHERE userid=$userid");
		if($r && $r['data']) {
			$cart = unserialize($r['data']);
			set_cookie('cart', count($cart), AJ_TIME + 30*86400);
		}
	}
	...
	function bind($username, $addtime) {
		...
		if($auth) {
			set_cookie('bind', '');
			$auth = decrypt($auth, AJ_KEY.'BIND');
			if(strpos($auth, '|') !== false) {
				$t = explode('|', $auth);
				$itemid = intval($t[0]);
				$site = check_name($t[1]) ? $t[1] : '';
				if($itemid && $site) {
					DB::query("UPDATE ".AJ_PRE."oauth SET username='' WHERE username='$username' AND site='$site'");
					DB::query("UPDATE ".AJ_PRE."oauth SET username='$username',addtime='$addtime' WHERE itemid=$itemid");
				}
			}
		}
		...
		if($auth) {
			...
			if(is_openid($openid)) {
				DB::query("UPDATE ".AJ_PRE."weixin_user SET username='' WHERE username='$username'");
				DB::query("UPDATE ".AJ_PRE."weixin_user SET username='$username',edittime='$addtime' WHERE openid='$openid'");
			}
		}
	}
	...
	function delete($userid) {
		...
		while($r = DB::fetch_array($result)) {
			...
			DB::query("DELETE FROM ".AJ_PRE."cart WHERE userid='$userid'");
			DB::query("DELETE FROM ".AJ_PRE."type WHERE item='friend-".$userid."'");
			DB::query("DELETE FROM ".AJ_PRE."type WHERE item='favorite-".$userid."'");
			DB::query("DELETE FROM ".AJ_PRE."type WHERE item='product-".$userid."'");
			DB::query("DELETE FROM ".AJ_PRE."type WHERE item='news-".$userid."'");
			DB::query("DELETE FROM ".AJ_PRE."type WHERE item='mall-".$userid."'");
			userclean($username);
	...
	function deluser($table, $user, $name = true, $data = false, $moduleid = 0) {
		...
		if($data) {
			$result = DB::query("SELECT * FROM ".AJ_PRE."{$table} WHERE `$fields`='$user'");
			while($r = DB::fetch_array($result)) {
				$itemid = $r['itemid'];
				DB::query("DELETE FROM ".AJ_PRE."{$table} WHERE itemid='$itemid'");
				...
				if($module == 'sell') {
					DB::query("DELETE FROM ".AJ_PRE."sell_search_{$moduleid} WHERE itemid=$itemid");
				}
			...
			DB::query("DELETE FROM ".AJ_PRE."{$table} WHERE `$fields`='$user'");
		}
	}

	function delupload($username, $userid) {
		if(!$userid || !$username) return;
		$result = DB::query("SELECT fileurl FROM ".AJ_PRE."upload_".($userid%10)." WHERE username='$username'");
		while($r = DB::fetch_array($result)) {
	...
	function rename($cusername, $nusername) {
		...
		foreach($MODULE as $m) {
			...
			if($m['module'] == 'group') {
				DB::query("UPDATE ".AJ_PRE."group_order_".$m['moduleid']." SET buyer='$nusername' WHERE buyer='$cusername'");
				DB::query("UPDATE ".AJ_PRE."group_order_".$m['moduleid']." SET seller='$nusername' WHERE seller='$cusername'");
			}
		}
		foreach($tables as $table) {
			DB::query("UPDATE ".AJ_PRE."{$table} SET username='$nusername' WHERE username='$cusername'");
		}
		DB::query("UPDATE ".AJ_PRE."chat SET fromuser='$nusername' WHERE fromuser='$cusername'");
		DB::query("UPDATE ".AJ_PRE."chat SET touser='$nusername' WHERE touser='$cusername'");
		DB::query("UPDATE ".AJ_PRE."order SET buyer='$nusername' WHERE buyer='$cusername'");
		DB::query("UPDATE ".AJ_PRE."order SET seller='$nusername' WHERE seller='$cusername'");
		DB::query("UPDATE ".AJ_PRE."message SET fromuser='$nusername' WHERE fromuser='$cusername'");
		DB::query("UPDATE ".AJ_PRE."message SET touser='$nusername' WHERE touser='$cusername'");
		userclean($cusername);
		return true;
	}
	...
	function edit_passport($cpassport, $npassport, $username) {
		...
		DB::query("UPDATE ".AJ_PRE."member SET passport='$npassport' WHERE passport='$cpassport'");
		DB::query("UPDATE ".AJ_PRE."comment SET passport='$npassport' WHERE passport='$cpassport'");
		foreach($MODULE as $m) {
			if($m['islink'] || $m['moduleid'] < 5) continue;
	...
	function login_log($username, $password, $salt, $admin = 0, $message = '') {
		...
		$port = isset($_SERVER['REMOTE_PORT']) ? intval($_SERVER['REMOTE_PORT']) : '';
		$agent = addslashes(dhtmlspecialchars(strip_sql(strip_tags($_SERVER['HTTP_USER_AGENT']))));
		$message or $message = $L['member_login_ok'];
		if($message == $L['member_login_ok']) cache_delete(AJ_IP.'.php', 'ban');
		DB::query("INSERT INTO ".AJ_PRE."login (username,password,passsalt,admin,loginip,loginport,logintime,message,agent) VALUES ('$username','$password','$salt','$admin','".AJ_IP."','$port','".AJ_TIME."','$message','$agent')");
	}
	...
	function check_add($post) {
		...
		DB::query("REPLACE INTO {$this->table_member_check} (userid,username,content,addtime) VALUES ('$this->userid','$this->username','$content','".AJ_TIME."')");
	}
