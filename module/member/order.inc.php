	...
	switch($step) {
		...
		case 'pay'://买家付款
			...
			if($submit) {
				...
				foreach($lists as $k=>$v) {
					$db->query("UPDATE {$table} SET status=2,updatetime=$AJ_TIME WHERE itemid=$v[itemid]");
				}
				$touser = $td['seller'];
		...
		case 'refund'://买家退款
			...
			if($submit) {
				...
				is_payword($_username, $password) or message($L['error_payword']);
				foreach($lists as $k=>$v) {
					$db->query("UPDATE {$table} SET status=5,updatetime=$AJ_TIME,buyer_reason='$content' WHERE itemid=$v[itemid]");
				}
		...
		case 'comment'://交易评价
			...
			if($submit) {
				foreach($lists as $k=>$v) {
					...
					$s = 's'.$star;
					$db->query("UPDATE {$AJ_PRE}mall_comment_".$mid." SET seller_star=$star,seller_comment='$content',seller_ctime=$AJ_TIME WHERE itemid=$itemid");
					$db->query("UPDATE {$AJ_PRE}mall_stat_".$mid." SET scomment=scomment+1,`$s`=`$s`+1 WHERE mallid=$mallid");
				}
		...
		case 'comment_detail'://评价详情
			...
			if($submit) {
				...
				if(isset($C[$oid])) {
					...
					$db->query("UPDATE {$AJ_PRE}mall_comment_".$O[$oid]['mid']." SET seller_reply='$content',seller_rtime=$AJ_TIME WHERE itemid=$oid");
		...
		case 'close'://关闭交易
			if($td['status'] == 0) {
				foreach($lists as $k=>$v) {
					$db->query("UPDATE {$table} SET status=8,updatetime=$AJ_TIME WHERE itemid=$v[itemid]");
				}
