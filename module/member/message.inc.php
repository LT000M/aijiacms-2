...
switch($action) {
	...
	case 'refuse':
		...
		$black = $db->get_one("SELECT black FROM {$AJ_PRE}member_misc WHERE userid=$_userid");
		$black = $black['black'];
		if($black) {
		...
		$db->query("UPDATE {$AJ_PRE}member_misc SET black='$black' WHERE userid=$_userid");
		userclean($_username);
	...
	case 'setting':
		if($submit) {
			...
			$send = $send ? 1 : 0;
			$db->query("UPDATE {$AJ_PRE}member_misc SET black='$black',send='$send' WHERE userid=$_userid");
			userclean($_username);
			dmsg($L['op_update_success'], '?action=setting');
		} else {
			$head_title = $L['message_title_black'].$AJ['seo_delimiter'].$head_title;
			$user = $db->get_one("SELECT black,send FROM {$AJ_PRE}member_misc WHERE userid=$_userid");
			$could_send = false;
			if($AJ['message_email'] && $AJ['mail_type'] != 'close') {
				if(check_group($_groupid, $AJ['message_group'])) $could_send = true;
			}
		}
	break;
	case 'outbox':
	...
	case 'last':
		if($_message) {
			$item = $db->get_one("SELECT itemid,feedback FROM {$AJ_PRE}message WHERE touser='$_username' AND status=3 AND isread=0 ORDER BY itemid DESC");
	...
	default:
		if($MG['inbox_limit']) {
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}message WHERE touser='$_username' AND status=3");
			$limit_used = $r['num'];
