...
login();
($MG['biz'] && $MG['homepage'] && $MG['honor_limit'] > -1) or dalert(lang('message->without_permission_and_upgrade'), 'goback');
if($MG['type'] && !$_edittime && $action == 'add') dheader($MODULE[2]['linkurl'].'edit.php?tab=2');
require AJ_ROOT.'/module/'.$module.'/common.inc.php';
require AJ_ROOT.'/include/post.func.php';
include load('my.lang');
require AJ_ROOT.'/module/'.$module.'/honor.class.php';
$do = new honor();
switch($action) {
	case 'add':
		...
		if($MG['hour_limit']) {
			$today = $AJ_TIME - 3600;
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}honor WHERE username='$_username' AND addtime>$today");
		...
		if($MG['honor_limit']) {
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}honor WHERE username='$_username' AND status>0");
			if($r['num'] >= $MG['honor_limit']) dalert(lang($L['limit_add'], array($MG['honor_limit'], $r['num'])), '?action=index');
		}
		if($submit) {
			if($do->pass($post)) {
...
$limit_used = 0;
for($i = 1; $i < 5; $i++) {
	$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}honor WHERE username='$_username' AND status=$i");
	$nums[$i] = $r['num'];
	$limit_used += $r['num'];
}
