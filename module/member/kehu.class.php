	...
	'已成交',
	   ...
	   if($post['status'] == 4) {
			...
			if(isset($post['cmoney']) && $post['cmoney']) {
					...
					amount_record($item['tuijian'], $item['truename'], $post['cmoney'],'', '0', $post['note'],$_username);
				}
			}
		}
...
function fenxiaoqy($areaid) {
	...
	if($areaid)$result = $db->query("SELECT areaid,areaname FROM {$db->pre}area where $condition");
...
function fxlp($house) {
	global $db, $dc, $CFG;
  if($house) $r = $db->get_one("SELECT * FROM {$db->pre}newhouse_6 WHERE itemid=$house ");
      $title=$r['title'];
	return $title;
}
?>