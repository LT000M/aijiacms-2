...
require AJ_ROOT.'/module/'.$module.'/addmember.class.php';
$do = new member();

switch($action) {
	case 'add':
			if($MG['addmember_limit']) {
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}member WHERE companyid='$_userid");
