...
if($submit) {
	...
	if($do->edit($post)) {
		...
		if($post['password']) dheader($AJ['file_login'].'?auth='.encrypt('LOGIN|'.$_username.'|'.$post['password'].'|'.$AJ_TIME, AJ_KEY.'LOGIN').'&forward='.urlencode($MOD['linkurl'].'edit.php?success=1&tab='.$tab));
