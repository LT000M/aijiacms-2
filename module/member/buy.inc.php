...
if(is_array($itemid) && !$_userid) {
	$AJ_URL = $MODULE[2]['linkurl'].'buy.php?action=add&mid='.$mid;
	foreach($itemid as $id) {
		$AJ_URL .= '&itemid[]='.$id;
	}
}
require AJ_ROOT.'/include/post.func.php';
require AJ_ROOT.'/module/mall/global.func.php';
include load('misc.lang');
include load('member.lang');
include load('order.lang');

if($submit) {
	require AJ_ROOT.'/module/member/cart.class.php';
	$do = new cart();
	$cart = $do->get();
	$ids = '';
	if($post) {
			...
			if($t && $t['status'] == 3 && $t['username'] != $_username && $t['amount'] > 0) {
				...
				if($express && in_array($express, array(1,2,3)) && $MOD['module'] == 'mall') {
					...
					if($express_id && $area_id) {
						$E = $db->get_one("SELECT * FROM {$AJ_PRE}mall_express_{$_mid} WHERE itemid=$express_id");
						if($E && $E['items'] > 0) {
							$AREA = cache_read('area.php');
							$aid = $area_id;
							$ii = 0;
							do {
								$E = $db->get_one("SELECT * FROM {$AJ_PRE}mall_express_{$_mid} WHERE parentid=$express_id AND areaid=$aid");
								if($E) {
				...
				if(isset($N[$t['username']])) {
					$pid = $N[$t['username']];
					$db->query("INSERT INTO {$AJ_PRE}order (mid,mallid,pid,buyer,seller,title,thumb,price,number,amount,addtime,updatetime,note, buyer_postcode,buyer_address,buyer_name,buyer_mobile,status,fee_name,fee,cod) VALUES ('$moduleid','$itemid','$pid','$_username','$t[username]','$title','$t[thumb]','$price','$number','$amount','$AJ_TIME','$AJ_TIME','$note','$buyer_postcode','$buyer_address','$buyer_name','$buyer_mobile','$status','$fee_name','$fee','$cod')");
					$oid = $db->insert_id();
					$db->query("UPDATE {$AJ_PRE}order SET `amount`=`amount`+$amount,`fee`=`fee`+$fee WHERE itemid=$pid");
					if($fee_name) $db->query("UPDATE {$AJ_PRE}order SET `fee_name`='$fee_name' WHERE itemid=$pid");
				} else {
					$db->query("INSERT INTO {$AJ_PRE}order (mid,mallid,buyer,seller,title,thumb,price,number,amount,addtime,updatetime,note, buyer_postcode,buyer_address,buyer_name,buyer_mobile,status,fee_name,fee,cod) VALUES ('$moduleid','$itemid','$_username','$t[username]','$title','$t[thumb]','$price','$number','$amount','$AJ_TIME','$AJ_TIME','$note','$buyer_postcode','$buyer_address','$buyer_name','$buyer_mobile','$status','$fee_name','$fee','$cod')");
					$pid = $db->insert_id();
					if(!$cod) {
						$N[$t['username']] = $pid;
						$ids .= ','.$pid;
					...
					$content = lang($L['trade_message_c6'], array(userurl($_username), $_username, timetodate($AJ_TIME, 3), $goods, $pid, $amount, $url));
					$content = ob_template('messager', 'mail');
					send_message($touser, $title, $content);
				}
				if($MODULE[$_mid]['module'] == 'mall') {
					$db->query("REPLACE INTO {$AJ_PRE}mall_comment_{$_mid} (itemid,mallid,buyer,seller) VALUES ('$oid','$itemid','$_username','$t[username]')");
					$tmp = $db->get_one("SELECT mallid FROM {$AJ_PRE}mall_stat_{$_mid} WHERE mallid=$itemid");
					if(!$tmp) $db->query("REPLACE INTO {$AJ_PRE}mall_stat_{$_mid} (mallid,buyer,seller) VALUES ('$itemid','$_username','$t[username]')");
				}
				unset($cart[$k]);
			}
		}
		if($N && $coupon) {
			foreach($N as $seller=>$oid) {
				if(isset($coupon[$seller])) {
					$cid = $coupon[$seller];
					$c = $db->get_one("SELECT * FROM {$AJ_PRE}finance_coupon WHERE itemid=$cid");
					if($c && $c['username'] == $_username && $c['seller'] == $seller && $c['oid'] == 0 && $c['fromtime'] < $AJ_TIME && $c['totime'] > $AJ_TIME) {
						$o = $db->get_one("SELECT * FROM {$AJ_PRE}order WHERE itemid=$oid");
						if($o && $o['buyer'] == $_username && $o['seller'] == $seller && $o['cid'] == 0 && $o['discount'] < 0.01) {
							$discount = $c['price'];
							if($c['cost'] <= ($o['amount'] + $o['fee']) && $c['price'] < $o['amount']) {
								$db->query("UPDATE {$AJ_PRE}order SET `amount`=`amount`-$discount,discount=$discount,cid=$cid WHERE itemid=$oid");
								$db->query("UPDATE {$AJ_PRE}finance_coupon SET oid=$oid WHERE itemid=$cid");
							}
						}
					}
				}
	...
	dheader('?action=show&auth='.encrypt($forward, AJ_KEY.'TURL'));
} else {
	$lists = $address = array();
	if($action == 'show') {
		$forward = isset($auth) ? decrypt($auth, AJ_KEY.'TURL') : '';
...
if($AJ_PC) {
	...
	$foot = '';
	if($action == 'show') {
		$back_link = AJ_MOB.'api/redirect.php?mid='.$moduleid.'&itemid='.$code;
	} else {		
