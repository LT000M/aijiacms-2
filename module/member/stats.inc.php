...
login();
($MG['biz'] && $MG['homepage'] && $MG['stats_view']) or dheader(($AJ_PC ? $MOD['linkurl'] : $MOD['mobile']).'account.php?action=group&itemid=1');
...
switch($action) {
	case 'update':
		...
		isset($tid) or $tid = '';
		if(!is_date($fid)) {
			$r = $db->get_one("SELECT max(id) AS fid FROM {$AJ_PRE}stats_user WHERE username='$_username'");
			if($r) {
			...
			is_date($fid) or $fid = timetodate(AJ_TIME - 86400*30, 3);
		}
		if(!is_date($tid)) {
			$tid = timetodate(AJ_TIME - 86400, 3);
		}
		if($fid >= $tid) message($L['op_update_success'], '?action=index');
		$id = str_replace('-', '', $tid);
		$t = $db->get_one("SELECT itemid FROM {$AJ_PRE}stats_user WHERE id='$id' AND username='$_username'");
		if(!$t) {
			...
			$db->query("INSERT INTO {$AJ_PRE}stats_user (username,id,ip,ip_pc,ip_mb,pv,pv_pc,pv_mb,rb,rb_pc,rb_mb) VALUES ('$_username','$id','$ip','$ip_pc','$ip_mb','$pv','$pv_pc','$pv_mb','$rb','$rb_pc','$rb_mb')");
		}
	...
	case 'report':
		...
		if($job == 'pv') {
			$data = $pv = $pv_pc = $pv_mb = '';
			$result = $db->query("SELECT * FROM {$AJ_PRE}stats_user WHERE username='$_username' AND id>$fromtime AND id<=$totime ORDER BY id ASC LIMIT 30", 'CACHE');
			...
			$xd = $yd = '';
			$lists = array();
			$result = $db->query("SELECT COUNT(`{$key}`) AS num,`{$key}` FROM {$AJ_PRE}stats_pv WHERE {$condition} GROUP BY `{$key}` ORDER BY num DESC LIMIT 0,50", 'CACHE');
			while($r = $db->fetch_array($result)) {
				if(!$r[$key]) continue;
				$r['url'] = '?action=record&'.$job.'='.urlencode($r[$key]);
				if($job == 'username') {
					$r['url'] = AJ_PATH.'api/redirect.php?username='.$r[$key];
				} else if($job == 'itemid') {
					$itemid = $r[$key];
					$t = $db->get_one("SELECT title FROM ".get_table($mid)." WHERE itemid=$itemid");
					if($t) {
						$r[$key] = dsubstr($t['title'], 30, '...');
						$r['url'] = AJ_PATH.'api/redirect.php?mid='.$mid.'&itemid='.$itemid;
					}
				}
				$lists[] = $r;
			}
	...
	case 'record':
		...
		$lists = $areas = array();
		$result = $db->query("SELECT * FROM {$AJ_PRE}stats_pv WHERE $condition ORDER BY sid DESC LIMIT $offset,$pagesize");
		while($r = $db->fetch_array($result)) {
			if(isset($areas[$r['ip']])) {
				$r['area'] = $areas[$r['ip']];
			} else {
	...
	break;
	default:
		$id = timetodate(AJ_TIME - 86400, 'Ymd');
		$t = $db->get_one("SELECT itemid FROM {$AJ_PRE}stats_user WHERE id='$id' AND username='$_username'");
		...
		if($page > 1 && $sum) {
			...
			$r = $db->get_one("SELECT COUNT(*) AS num FROM {$AJ_PRE}stats_user WHERE $condition");
			$items = $r['num'];
		}
		$pages = $AJ_PC ? pages($items, $page, $pagesize) : mobile_pages($items, $page, $pagesize);
		$lists = array();
		$result = $db->query("SELECT * FROM {$AJ_PRE}stats_user WHERE $condition ORDER BY $dorder[$order] LIMIT $offset,$pagesize");
		while($r = $db->fetch_array($result)) {
