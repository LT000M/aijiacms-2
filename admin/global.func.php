...
function update_setting($item, $setting) {
	DB::query("DELETE FROM ".AJ_PRE."setting WHERE item='$item'");
	foreach($setting as $k=>$v) {
		if(is_array($v)) $v = implode(',', $v);		
		$v = addslashes(stripslashes($v));
		DB::query("INSERT INTO ".AJ_PRE."setting (item,item_key,item_value) VALUES ('$item','$k','$v')");
	}
	return true;
}
...
function get_setting($item) {
	$setting = array();
	$query = DB::query("SELECT * FROM ".AJ_PRE."setting WHERE item='$item'");
...
function update_category($CAT) {
	...
	DB::query("UPDATE ".AJ_PRE."category SET linkurl='$linkurl' WHERE catid=".$CAT['catid']);
}
...
function fetch_url($url) {
	...
	$tmp = parse_url($url);
	$domain = $tmp['host'];
	$r = DB::get_one("SELECT * FROM ".AJ_PRE."fetch WHERE domain='$domain' ORDER BY edittime DESC");
	if($r) {
...
function admin_log($force = 0) {
	global $AJ, $AJ_QST, $moduleid, $file, $action, $_username;
	...
	if($AJ['admin_log'] == 2 || ($AJ['admin_log'] == 1 && ($file == 'setting' || in_array($action, array('delete', 'edit', 'move', 'clear', 'add'))))) {
		...
		DB::query("INSERT INTO ".AJ_PRE."admin_log(qstring,username,ip,logtime) VALUES('$AJ_QST','$_username','".AJ_IP."','".AJ_TIME."')");
		set_cookie('logstring', $md5string);
	}
}

function admin_online() {
	global $AJ, $AJ_QST, $moduleid, $_username;
	...
	DB::query("REPLACE INTO ".AJ_PRE."admin_online (sid,username,ip,moduleid,qstring,lasttime) VALUES ('".session_id()."','$_username','".AJ_IP."','$moduleid','$qstring','".AJ_TIME."')");	
...
function admin_check() {
	...
	if($_admin == 2) {
		...
		if($catid) {
			...
			//Childs
			$result = DB::query("SELECT catid,child,arrchildid FROM ".AJ_PRE."category WHERE moduleid=$moduleid AND catid IN ($_catids)");
...
function admin_notice() {
	global $AJ, $MODULE, $moduleid, $file, $itemid, $action, $reason, $msg, $eml, $sms, $wec;
	...
	if($moduleid > 4) {
		...
		if($MODULE[$moduleid]['module'] == 'job') {
			if($file == 'resume') {
				$table = AJ_PRE.'job_'.$file.'_'.$moduleid;
				$name = '简历';
			} else {
				$name = '招聘';
			}
...
function split_sell($part) {
	...
	$sql = str_replace('aijiacms_sale', AJ_PRE.'sale_5_'.$part, $sql);
	if(DB::version() > '4.1' && $CFG['db_charset']) {
...
function install_file($moduleid, $file, $dir, $extend = 0) {
	...
	$content .= '?>';
	file_put(AJ_ROOT.'/'.$dir.'/'.$file.'.php', $content);
	$content = "<?php\n";
	if($extend == 1) $content .= "define('AJ_REWRITE', true);\n";
	$content .= "\$moduleid = ".$moduleid.";\n";
	$content .= "require '../../common.inc.php';\n";
	$content .= "require AJ_ROOT.'/include/mobile.inc.php';\n";
	$content .= "require AJ_ROOT.'/module/'.\$module.'/".$file.".inc.php';\n";
	$content .= '?>';
	file_put(AJ_ROOT.'/mobile/'.$dir.'/'.$file.'.php', $content);
}
