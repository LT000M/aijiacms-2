	...
	case 'edit':
		if($submit) {
			...
			if($areaid) {
				...
				$areaid = $listorder = 0;
				$name = $style = $letter = $domain = $iparea = $template = $seo_title = $seo_keywords = $seo_description = $map_mid = $map_mid = '';'';
			}
			include tpl('city_edit');
		}
	break;
	case 'letter':
		$result = $db->query("SELECT * FROM {$AJ_PRE}city WHERE letter=''");
		while($r = $db->fetch_array($result)) {
			$letter = $do->letter($r['name']);
			$db->query("UPDATE {$AJ_PRE}city SET letter='$letter' WHERE areaid=$r[areaid]");
		}
		dmsg('更新成功', $forward);
	break;
	case 'delete':
...
class city {
	...
	function get_list($condition) {
		...
		while($r = DB::fetch_array($result)) {
			$r['linkurl'] = AJ_PATH.'api/city.php?action=go&forward=&areaid='.$r['areaid'];
			$lists[] = $r;
		}
		return $lists;
	}
