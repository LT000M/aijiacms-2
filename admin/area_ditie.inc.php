...
switch($action) {
	case 'add':
		if($submit) {
			...
			include tpl('area_ditie_add');
		}
	break;
	case 'import':
		$file = AJ_ROOT.'/file/setting/area.sql';
...
class area {
	...
	function __construct($areaid = 0)	{
		...
		$this->areaid = $areaid;
		$this->area = $AREA;
		$this->table = 'aijiacms_area_ditie'; // 修改为新的数据表
	}
	...
	function update($area) {
		...
		foreach($area as $k=>$v) {
			...
			DB::query("UPDATE {$this->table} SET  areaname='$v[areaname]',parentid='$v[parentid]',listorder='$v[listorder]',map='$v[map]' WHERE areaid=$k");
		}
		cache_area_ditie();
		return true;
	}
