...
switch($action) {
	case 'repair':
		$AJ['close'] or msg('为了数据安全，此操作必须在网站设置里关闭网站');
	...
	case 'comments':
		...
		foreach($MODULE as $k=>$v) {
			if(in_array($v['module'], array('article', 'newhouse', 'buy', 'rent', 'info', 'photo', 'sale', 'video'))) {
				$sql = "ALTER TABLE `".$AJ_PRE.$v['module']."_".$v['moduleid']."` COMMENT='".$v['name']."'";
				$db->query($sql);
				$sql = "ALTER TABLE `".$AJ_PRE.$v['module']."_data_".$v['moduleid']."` COMMENT='".$v['name']."内容'";
				$db->query($sql);
			}
		}
		dmsg('重建成功', '?file='.$file);
			...
			if(isset($filename) && $filename && file_ext($filename) == 'sql') {
				...
				if(is_file($filename)) {
					...
					if(substr($sql, 0, 11) == '# AIJIACMS V') {
						$v = substr($sql, 11, 3);
						if(AJ_VERSION != $v) msg('由于数据结构存在差异，备份数据不可以跨版本导入<br/>备份版本：V'.$v.'<br/>当前系统：V'.AJ_VERSION);
					}
					sql_execute($sql);
	...
	default:
		if(isset($backup)) {
			...
			if(trim($sqldump)) {
				$sqldump = "# AIJIACMS V".AJ_VERSION." R".AJ_RELEASE." https://www.aijiacms.com\n# ".timetodate($AJ_TIME, 6)."\n# --------------------------------------------------------\n\n\n".$sqldump;
				$tableid = $i;
