...
class admin {
	...
	function count_admin() {
		$r = DB::get_one("SELECT COUNT(*) AS num FROM ".AJ_PRE."member WHERE groupid=1 AND admin=1 ");
		return $r['num'];
	}
	...
	function set_admin($username, $admin, $role, $aid) {
		...
		if($admin == 1) $aid = 0;
		DB::query("UPDATE ".AJ_PRE."member SET groupid=1,admin=$admin,role='$role',aid=$aid WHERE userid=$userid");
		DB::query("UPDATE ".AJ_PRE."company SET groupid=1 WHERE userid=$userid");
		return true;
	}
	...
	function move_admin($username) {
		...
		if($r && $r['admin'] > 0) {			
			...
			$admin = $r['admin'] == 1 ? 2 : 1;
			DB::query("UPDATE ".AJ_PRE."member SET admin=$admin WHERE username='$username'");
			return true;
		} else {
	...
	function delete_admin($username) {
		...
		if($r) {
			...
			DB::query("UPDATE ".AJ_PRE."member SET groupid=$groupid,admin=0,role='',aid=0 WHERE userid=$userid");
			DB::query("UPDATE ".AJ_PRE."company SET groupid=$groupid WHERE userid=$userid");
			DB::query("DELETE FROM ".AJ_PRE."admin WHERE userid=$userid");
	...
	function get_one($user, $type = 1) {
        ...
        return DB::get_one("SELECT * FROM ".AJ_PRE."member WHERE `$fields`='$user'");
	}
	...
	function get_list($condition) {
		...
		$admins = array();
		$result = DB::query("SELECT * FROM ".AJ_PRE."member WHERE $condition ORDER BY admin ASC,userid ASC LIMIT $offset,$pagesize");
	...
	function get_right($userid) {
		global $MODULE;
		$rights = array();
		$result = DB::query("SELECT * FROM ".AJ_PRE."admin WHERE userid=$userid AND url='' ORDER BY moduleid DESC,file DESC,adminid DESC ");
		while($r = DB::fetch_array($result)) {
			@include AJ_ROOT.'/'.($r['moduleid'] == 1 ? 'admin' : 'module/'.$MODULE[$r['moduleid']]['module'].'/admin').'/config.inc.php';
	...
	function get_menu($userid) {
		$menus = array();
		$result = DB::query("SELECT * FROM ".AJ_PRE."admin WHERE userid=$userid AND url!='' ORDER BY listorder ASC,adminid ASC ");
		while($r = DB::fetch_array($result)) {
			$menus[] = $r;
		}
		return $menus;
	...
	function update($userid, $right, $admin) {
		...
		if($admin == 1) DB::query("DELETE FROM ".AJ_PRE."admin WHERE userid=$userid AND url=''");
	...
	function add($userid, $right, $admin) {
		if(isset($right['url'])) {
			...
			$r = DB::get_one("SELECT * FROM ".AJ_PRE."admin WHERE userid=$userid AND url='".$right['url']."'");
			if($r) return false;
        ...
        $sql1 = substr($sql1, 1);
        $sql2 = substr($sql2, 1);
		DB::query("INSERT INTO ".AJ_PRE."admin ($sql1) VALUES($sql2)");
	}
	...
	function edit($right, $type = 0) {
		...
		foreach($right as $key=>$value) {
			...
			$sql = substr($sql, 1);
			DB::query("UPDATE ".AJ_PRE."admin SET $sql WHERE adminid='$key'");
		}
	}
	...
	function delete($adminid) {
		DB::query("DELETE FROM ".AJ_PRE."admin WHERE adminid=$adminid");
	}
