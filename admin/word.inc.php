...
switch($action) {
	case 'upload':
		...
		if($upload->save()) {
			...
			$F = get_file(AJ_ROOT.'/file/temp/'.$name);
			if($F) {
				$htm = '';
				$max = 0;
				foreach($F as $f) {
					...
					if(in_array($ext, array('htm', 'html'))) {
						$tmp = filesize($f);
						if($tmp > $max) $htm = str_replace(AJ_ROOT.'/file/temp/', '', $f);
						$max = $tmp;
					} else {
	...
	case 'read':
		if($word && in_array(file_ext($word), array('htm', 'html'))) {
			$data = file_get(AJ_ROOT.'/file/temp/'.$word);
			if($data) {
				if($charset) $data = convert($data, $charset, AJ_CHARSET);
	...
	case 'run':
		...
		$base = AJ_PATH.'file/temp/'.dirname($temp).'/';
		$code = str_replace('src="', 'src="'.$base, $code);
		echo '<html>';
		echo '<meta http-equiv="Content-Type" content="text/html;charset='.AJ_CHARSET.'"/>';
		echo '<body>';
		echo $code;
		echo '</body>';
		echo '</html>';
