layui.define(["element", "jquery"], function (exports) {
    ...
    layuih = new function () {
        ...
        this.init = function (url) {
            ...
            $.getJSON(url, function (data, status) {
                if (data == null) {
                    layuih.msg_error('暂无菜单信息');
                } else {
                    layuih.initHome(data.homeInfo);
                    layuih.initLogo(data.logoInfo);
                    layuih.initClear(data.clearInfo);
                    layuih.initMenu(data.menuInfo);
                    layuih.initTab();
                }
            }).fail(function () {
                layuih.msg_error('菜单接口有误');
            });
            layer.close(loading);
        };
        ...
        this.initDevice = function () {
            if (layuih.checkMobile()) {
                $('.layuih-tool i').attr('data-side-fold', 0);
                $('.layuih-tool i').attr('class', 'fa fa-indent');
                $('.layui-layout-body').attr('class', 'layui-layout-body layuih-mini');
            }
        };
        ...
        this.initHome = function (data) {
            sessionStorage.setItem('layuihHomeHref', data.href);
            $('#layuihHomeTabId').html('<i class="' + data.icon + '"></i> <span>' + data.title + '</span>');
            $('#layuihHomeTabId').attr('lay-id', data.href);
            $('#layuihHomeTabIframe').html('<iframe width="100%" height="100%" frameborder="0"  src="' + data.href + '"></iframe>');
        };
        ...
        this.initClear = function (data) {
            $('.layuih-clear').attr('data-href', data.clearUrl);
        };
        ...
        this.initBgColor = function () {
            var bgcolorId = sessionStorage.getItem('layuihBgcolorId');
            if (bgcolorId == null || bgcolorId == undefined || bgcolorId == '') {
                bgcolorId = 0;
            }
            var bgcolorData = layuih.bgColorConfig(bgcolorId);
            var styleHtml = '.layui-layout-admin .layui-header{background-color:' + bgcolorData.headerRight + '!important;}\n' +
                '.layui-header>ul>.layui-nav-item.layui-this,.layuih-tool i:hover{background-color:' + bgcolorData.headerRightThis + '!important;}\n' +
                ...
                '}';
            $('#layuih-bg-color').html(styleHtml);
        };
        ...
        this.initTab = function () {
            ...
            if (urlArr.length >= 2) {
                ...
                // 判断链接是否有效
                var checkUrl = layuih.checkUrl(href);
                if (checkUrl != true) {
                    return layuih.msg_error(checkUrl);
                }

                // 判断tab是否存在
                var checkTab = layuih.checkTab(href);
                if (!checkTab) {
                    var title = href,
                        tabId = href;
                    ...
                    layuihHomeTab = $('#layuihHomeTab').attr('lay-id');
                    layuihHomeHref = sessionStorage.getItem('layuihHomeHref');
                    if (layuihHomeTab != href && layuihHomeHref != href) {
                        layuih.addTab(tabId, href, title, true);
                        layuih.changeTab(tabId);
                    }
                }
            }
            layuih.hashTab();
        ...
        this.buildBgColorHtml = function () {
            ...
            var bgColorConfig = layuih.bgColorConfig();
        ...
        this.checkTab = function (tabId, isIframe) {
            ...
            // 判断sessionStorage是否有
            var layuihTabInfo = JSON.parse(sessionStorage.getItem("layuihTabInfo"));
            if (layuihTabInfo == null) {
                layuihTabInfo = {};
            }
            var check = layuihTabInfo[tabId];
        ...
        this.addTab = function (tabId, href, title, addSession) {
            if (addSession == undefined || addSession == true) {
                ...
                layuihTabInfo[tabId] = {href: href, title: title}
                sessionStorage.setItem("layuihTabInfo", JSON.stringify(layuihTabInfo));
            }
            element.tabAdd('layuihTab', {
        ...
        this.delTab = function (tabId) {
            var layuihTabInfo = JSON.parse(sessionStorage.getItem("layuihTabInfo"));
            if (layuihTabInfo != null) {
                delete layuihTabInfo[tabId];
                sessionStorage.setItem("layuihTabInfo", JSON.stringify(layuihTabInfo))
            }
            element.tabDelete('layuihTab', tabId);
        };
        ...
        this.changeTab = function (tabId) {
            element.tabChange('layuihTab', tabId);
        };
        ...
        this.hashTab = function () {
            ...
            element.tabChange('layuihTab', layId);
            element.on('tab(layuihTab)', function (elem) {
    ...
    $('body').on('click', '[data-tab]', function () {
        ...
        // 判断该窗口是否已经打开过
        var checkTab = layuih.checkTab(tabId);
        if (!checkTab) {
            layuih.addTab(tabId, href, title, true);
        }
        element.tabChange('layuihTab', tabId);
    ...
    $('body').on('click', '[data-iframe-tab]', function () {
        ...
        // 判断该窗口是否已经打开过
        var checkTab = layuih.checkTab(tabId, true);
        if (!checkTab) {
            var layuihTabInfo = JSON.parse(sessionStorage.getItem("layuihTabInfo"));
            if (layuihTabInfo == null) {
                layuihTabInfo = {};
            ...
            layuihTabInfo[tabId] = {href: href, title: title}
            sessionStorage.setItem("layuihTabInfo", JSON.stringify(layuihTabInfo));
            parent.layui.element.tabAdd('layuihTab', {
                title: title + '<i data-tab-close="" class="layui-icon layui-unselect layui-tab-close">ဆ</i>' //用于演示
                , content: '<iframe width="100%" height="100%" frameborder="0"  src="' + href + '"></iframe>'
                , id: tabId
            });
        }
        parent.layui.element.tabChange('layuihTab', tabId);
        layuih.tabRoll();
        parent.layer.close(loading);
    });
    ...
    $('body').on('click', '[data-clear]', function () {
        ...
        if (clearUrl != undefined && clearUrl != '' && clearUrl != null) {
            $.getJSON(clearUrl, function (data, status) {
                layer.close(loading);
                if (data.code != 1) {
                    return layuih.msg_error(data.msg);
                } else {
                    return layuih.msg_success(data.msg);
                }
            }).fail(function () {
                layer.close(loading);
                return layuih.msg_error('清理缓存接口有误');
            });
        } else {
            layer.close(loading);
            return layuih.msg_success('清除缓存成功');
        }
    });
    ...
    $('body').on('click', '[data-side-fold]', function () {
        ...
        if (isShow == 1) { // 缩放
            ...
            $('.layuih-tool i').attr('class', 'fa fa-outdent');
            $('.layui-layout-body').attr('class', 'layui-layout-body layuih-all');
        }
        layuih.tabRoll();
        element.init();
        layer.close(loading);
    ...
    $('body').on('click', '[data-bgcolor]', function () {
        ...
        var bgColorHtml = layuih.buildBgColorHtml();
        var html = '<div class="layuih-color">\n' +
    ...
    $('body').on('click', '[data-select-bgcolor]', function () {
        ...
        $('.layuih-color .color-content ul .layui-this').attr('class','');
        $(this).attr('class','layui-this');
        sessionStorage.setItem('layuihBgcolorId', bgcolorId);
        layuih.initBgColor();
    });

    exports("layuih", layuih);
});