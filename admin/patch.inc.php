...
if($action == 'view') {
	...
	is_dir(AJ_ROOT.'/file/patch/'.$fid) or msg();
	$lists = get_file(AJ_ROOT.'/file/patch/'.$fid);
	include tpl('patch_view');
} else if($action == 'delete') {
	isset($fid) or msg();
	preg_match("/^[0-9\.\-\s~]{33}$/", $fid) or msg();
	is_dir(AJ_ROOT.'/file/patch/'.$fid) or msg();
	dir_delete(AJ_ROOT.'/file/patch/'.$fid);
	dmsg('删除成功', '?file='.$file);
} else {
	if($submit) {
		...
		foreach($filedir as $d) {
			if($d == 'file') {
				$files = array_merge($files, get_file(AJ_ROOT.'/'.$d.'/script', $fileext));
				$files = array_merge($files, get_file(AJ_ROOT.'/'.$d.'/image', $fileext));
				$files = array_merge($files, get_file(AJ_ROOT.'/'.$d.'/config', $fileext));
			} else {
		...
		$lists = array();
		foreach($files as $f) {
			if(in_array($f, array(AJ_ROOT.'/file/script/config.js'))) continue;
			$n = basename($f);
			if(file_ext($n) == 'js') {
		...
		$find = count($lists);
		if($find) {
			$dir = AJ_ROOT.'/file/patch/'.timetodate($ft, 'Y-m-d H.i').'~'.timetodate($tt, 'Y-m-d H.i').'/';
			...
			foreach($lists as $f) {
				file_copy($f, $dir.str_replace(AJ_ROOT.'/', '', $f));
				@touch($dir.str_replace(AJ_ROOT.'/', '', $f), filemtime($f));
			}
		...
		$fd = substr(AJ_RELEASE, 0, 4).'-'.substr(AJ_RELEASE, 4, 2).'-'.substr(AJ_RELEASE, 6, 2).' 00:00:00';
		$tt = cache_read('patch.php');
		if($tt && is_time($tt[0])) $fd = $tt[0];
		$td = timetodate($AJ_TIME, 6);
		$files = glob(AJ_ROOT.'/file/patch/*');
		foreach($files as $f) {
			if(is_dir($f)) {
				$n = basename($f);
