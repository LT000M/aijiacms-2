...
$tb or msg();
$len = strlen($AJ_PRE);
if(substr($tb, 0, $len) == $AJ_PRE) $tb = substr($tb, $len);
$do = new fields();
$do->tb = $tb;
$menus = array (
...
class fields {
	...
	function add($post) {
		...
		$name = '`'.$post['name'].'`';
        DB::query("ALTER TABLE ".AJ_PRE."{$this->tb} ADD $name $type NOT NULL");
		$sqlk = $sqlv = '';
		foreach($post as $k=>$v) {
			$sqlk .= ','.$k; $sqlv .= ",'$v'";
		}
	...
	function edit($post) {
		...
		unset($post['cname']);
		$name = '`'.$post['name'].'`';
        DB::query("ALTER TABLE ".AJ_PRE."{$this->tb} CHANGE $cname $name $type NOT NULL");
		$sql = '';
		foreach($post as $k=>$v) {
			$sql .= ",$k='$v'";
		}
	...
	function delete($itemid) {
	    ...
	    DB::query("ALTER TABLE ".AJ_PRE."{$this->tb} DROP $name");
	}
