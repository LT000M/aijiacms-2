...
<?php } else if($mid == 3) { ?>
<option value="">--------------------</option>
<option value="?moduleid=<?php echo $mid;?>&file=qita">其他管理</option>
<?php
	include AJ_ROOT.'/module/extend/admin/menu.inc.php';
	foreach($menu as $m) {
...
<option value="">--------------------</option>
<?php
	include AJ_ROOT.'/module/member/admin/menu.inc.php';
	foreach($menu as $m) {
		if(strpos($m[1], 'setting') !== false) continue;
		echo '<option value="'.$m[1].'">'.$m[0].'</option>';
	}
...
<option value="">--------------------</option>
<?php
	include AJ_ROOT.'/module/company/admin/menu.inc.php';
	foreach($menu as $m) {
		if(strpos($m[1], 'setting') !== false) continue;
		echo '<option value="'.$m[1].'">'.$m[0].'</option>';
	}
...
<?php if($mid == 6) { ?>
<option value="?moduleid=2&file=house_groupbuy">团购管理</option>
<option value="?moduleid=3&file=wenfang">问房管理</option>
<option value="?moduleid=<?php echo $mid;?>&file=hongbao">红包管理</option>
<option value="?moduleid=2&file=house_kehu">分销客户管理</option>
<option value="?moduleid=2&file=amount">分销佣金管理</option>
<option value="?moduleid=<?php echo $mid;?>&file=xiangce">相册管理</option>
<option value="?moduleid=<?php echo $mid;?>&file=huxing">户型管理</option>
<option value="?moduleid=<?php echo $mid;?>&file=shapan">沙盘管理</option>
<option value="?moduleid=<?php echo $mid;?>&file=price">报价管理</option>
