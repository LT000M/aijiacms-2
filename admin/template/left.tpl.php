...
<!--[if lte IE 9]><!-->
<script type="text/javascript" src="<?php echo AJ_STATIC;?>file/script/jquery-1.5.2.min.js"></script>
<!--<![endif]-->
<!--[if (gte IE 10)|!(IE)]><!-->
<script type="text/javascript" src="<?php echo AJ_STATIC;?>file/script/jquery-2.1.1.min.js"></script>
<!--<![endif]-->
...
if($_admin == 2) {
?>
<table cellpadding="0" cellspacing="0" width="<?php echo $AJ['admin_left'];?>" height="100%">
<tr>
...
<div id="m_1">
	...
	?>
	<dd onclick="c(this);"><a href="<?php echo substr($menu['url'], 0, 1) == '?' ? $menu['url'] : AJ_PATH.'api/redirect.php?url='.$menu['url'].'" target="_blank';?>"><?php echo set_style($menu['title'], $menu['style']);?></a></dd>
	<?php
		}
	?>
	</dl>
...
<td valign="top" class="barmain" id="menu">
	...
	<?php
		foreach($mymenu as $m) {
	?>
	<dd onclick="c(this);"><a href="<?php echo substr($m['url'], 0, 1) == '?' ? $m['url'] : AJ_PATH.'api/redirect.php?url='.$m['url'].'" target="_blank';?>"><?php echo set_style($m['title'], $m['style']);?></a></dd>
	<?php
		}
	?>
	</dl>
	...
	<dd onclick="c(this);"><a href="<?php echo $MODULE[2]['linkurl'];?>" target="_blank">会员中心</a></dd>
	<dd onclick="c(this);"><a href="?file=logout" target="_top" onclick="return confirm('确定要退出管理后台吗');">安全退出</a></dd>
	</dl>
	<dl>
	<dt onclick="s(this)" onmouseover="this.className='dt_on';" onmouseout="this.className='';">分销楼盘</dt>
	<?php
	...
	?>
	<?php
		$menuinc = AJ_ROOT.'/module/'.$MODULE[4]['module'].'/admin/menu.inc.php';
		if(is_file($menuinc)) {
			extract($MODULE[4]);
			include $menuinc;
			echo '<dl id="dl_'.$moduleid.'">';
