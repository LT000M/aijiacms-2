...
switch($action) {
	case 'download':
		...
		$url = 'http://www.aijiacms.com/update.php?product=b2b&release='.$release.'&version='.AJ_VERSION.'&charset='.AJ_CHARSET.'&lang='.AJ_LANG.'&domain='.(AJ_DOMAIN ? AJ_DOMAIN : AJ_PATH);
		$code = dcurl($url);
		if($code) {
