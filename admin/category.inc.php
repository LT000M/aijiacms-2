...
switch($action) {
	...
	case 'count':
		...
		$tb = get_table($mid);
		if($MODULE[$mid]['module'] == 'club') $tb = $AJ_PRE.'club_group_'.$mid;
		if(!isset($num)) {
			$num = 50;
		}
		if(!isset($fid)) {
	...
	default:
		...
		if(!$AJCAT && !$parentid && !$keyword) msg('暂无分类,请先添加',  '?file='.$file.'&mid='.$mid.'&action=add&parentid='.$parentid);
